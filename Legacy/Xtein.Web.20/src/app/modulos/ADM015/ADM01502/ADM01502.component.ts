...
export class ADM01502Component implements OnInit {
  ...
  public _unsubscribeAll: Subject<any>;
    ...
    // Checkboxes de cabecera
  checkAllPermisos: boolean = false;
  checkAllAprobacion: boolean = false;
  ...
  onRowValidating(e: any) {
    const fila = { ...e.oldData, ...e.newData };
    if (fila.PERMISO && fila.APROBACION) {
      e.isValid = false;
      e.errorText = 'No se pueden activar Permisos y Apro. Permisos al mismo tiempo';
    }
  }

  onToggleAllPermisos(checked: boolean) {
    if (!this.esEdicion) return;
    if (checked && this.checkAllAprobacion) {
      notify('Solo se puede activar una columna a la vez: Permisos o Apro. Permisos', 'error', 3500);
      this.checkAllPermisos = false;
      return;
    }
    this.checkAllPermisos = checked;
    if (this.DPermisosEspeciales) {
      this.DPermisosEspeciales.forEach(row => row.PERMISO = checked);
      this.gridPermisosEspeciales.instance.refresh();
    }
  }

  onToggleAllAprobacion(checked: boolean) {
    if (!this.esEdicion) return;
    if (checked && this.checkAllPermisos) {
      notify('Solo se puede activar una columna a la vez: Permisos o Apro. Permisos', 'error', 3500);
      this.checkAllAprobacion = false;
      return;
    }
    this.checkAllAprobacion = checked;
    if (this.DPermisosEspeciales) {
      this.DPermisosEspeciales.forEach(row => row.APROBACION = checked);
      this.gridPermisosEspeciales.instance.refresh();
    }
  }
