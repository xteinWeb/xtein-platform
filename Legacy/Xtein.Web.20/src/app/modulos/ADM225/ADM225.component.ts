...
export class ADM225Component {
      ...
      this.sData
        ...
        .subscribe((data: any) => {
            ...
            this.loadingVisible = false;
                ...
                const datosFiltrados = this.agruparDatosVisibles(datares);

                if (datares.length > 0 && datosFiltrados.length === 0) {
                  this.showModal('No se puede visualizar este registro porque se encuentra en estado ANULADO.', 'Advertencia');
                  this.valoresObjetos('autorizaciones', '');
                  return;
                }
  ...
  opIrARegistro(accion: string): void {
    ...
    this.DgAutorizaciones = this.prepararDatosGrid(JSON.parse(JSON.stringify(newArray.DATOS)));
    this._sbarreg.setObsMenuReg(this.prmUsrAplBarReg);
  }
  ...
  private prepararDatosGrid(datos: any[]): any[] {
    return this.sanitizarDatos(datos).filter((row: any) => {
      const estado = (row.ESTADO ?? '').toString().trim().toUpperCase();
      return estado !== 'ANULADO';
    });
  }

  private agruparDatosVisibles(datos: any[]): any[] {
    const valoresUnicos = [...new Set(datos.map((objeto: any) => objeto.ID_APLICACION))];

    return valoresUnicos
      .map((valor) => {
        const datosAplicacion = this.prepararDatosGrid(
          datos.filter((objeto: any) => objeto.ID_APLICACION === valor)
        );
      ...
      .filter((item: any) => item.DATOS.length > 0);
  }
    ...
    if (obj === 'autorizaciones') {
      ...
      this.sData.consulta('AUTORIZACIONES PENDIENTES', prm, this.prmUsrAplBarReg.aplicacion).subscribe((data: any) => {
        ...
        if (mensaje !== '') {
          ...
          this.DgAutorizaciones = this.prepararDatosGrid(JSON.parse(JSON.stringify(this.DgAutorizaciones_prev)));
