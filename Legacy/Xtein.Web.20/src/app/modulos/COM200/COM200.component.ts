...
export class COM200Component implements OnInit {
  @ViewChild('formOrdenCompra', { static: false })
  formOrdenCompra: DxFormComponent;
  @ViewChild('gridOrdenCompraItems', { static: false })
  gridOrdenCompraItems: DxDataGridComponent;
  @ViewChild('xs_FECHA_ENTREGA', { static: false })
  fechaEntrega: DxDateBoxComponent;
  @ViewChild('checkProSoloPrecio', { static: false })
  checkProSoloPrecio: DxCheckBoxComponent;
  @ViewChild('checkProProveedor', { static: false })
  checkProProveedor: DxCheckBoxComponent;
  @ViewChild('xs_PRODUCTOS', { static: false })
  sboxProductos: DxSelectBoxComponent;
  @ViewChild('gridProLista', { static: false })
  gridProLista: DxDataGridComponent;
  @ViewChild('ddProductos', { static: false })
  ddProductos: DxDropDownBoxComponent;
  ...
  constructor(
    ...
    this.subs_visor = this.SVisor.getObs_Apl().subscribe((resp) => {
      ...
      const nx = this.VDatosReg.findIndex(
        (d) =>
          d.ID_DOCUMENTO === resp.ID_DOCUMENTO &&
          d.CONSECUTIVO === resp.CONSECUTIVO,
      );
      if (nx !== 0) {
      ...
      case 'r_cancelar':
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            this.esVisibleSelecc = 'none';
            this._sbarreg.setObsMenuReg(this.prmUsrAplBarReg);
            this.DOrdenCompra = JSON.parse(
              JSON.stringify(this.DOrdenCompra_prev),
            );
            this.DOrdenCompraItems = JSON.parse(
              JSON.stringify(this.DOrdenCompraItems_prev),
            );
            this.DOrdenCompraGrav = JSON.parse(
              JSON.stringify(this.DOrdenCompraGrav_prev),
            );
  ...
  opPrepararNuevo(): void {
    ...
    this.opBlanquearForma();
    if (!this.esInicioDatos) this.valoresObjetos('todos');
    else this.valoresObjetos('documento');
  ...
  valoresDefecto() {
    if (this.especAplicacion) {
      this.DOrdenCompra.ID_MONEDA =
        this.especAplicacion.find(
          (e: any) => e.NOMBRE_OBJETO === 'ID_MONEDA_DEF',
        )?.VALOR_DEFECTO ?? '';
      this.especMonedaFormato =
        this.especAplicacion.find(
          (e: any) => e.NOMBRE_OBJETO === 'FORMATO MONEDA',
        )?.FORMATO ?? '#,##0.00';
      this.especCantidadFormato =
        this.especAplicacion.find(
          (e: any) => e.NOMBRE_OBJETO === 'FORMATO CANTIDAD',
        )?.FORMATO ?? '#,##0.00';
      this.especModificarPrecio =
        this.especAplicacion.find(
          (e: any) => e.NOMBRE_OBJETO === 'MODIFICAR PRECIO',
        )?.ACTIVADO ?? false;
      this.especModificarGrav =
        this.especAplicacion.find(
          (e: any) => e.NOMBRE_OBJETO === 'MODIFICAR GRAVAMENES',
        )?.ACTIVADO ?? false;
      this.especRepetirItem =
        this.especAplicacion.find(
          (e: any) => e.NOMBRE_OBJETO === 'REPETIR ITEM',
        )?.ACTIVADO ?? false;
      this.especOrigenPrecio =
        this.especAplicacion.find(
          (e: any) => e.NOMBRE_OBJETO === 'MODIFICAR PRECIO',
        )?.VALOR_DEFECTO ?? '';

      // Cuentas de email usuario
      this.especEmailCompras =
        this.especAplicacion.find(
          (e: any) => e.NOMBRE_OBJETO === 'EMAIL COMPRAS',
        )?.VALOR_DEFECTO ?? '';
      this.especUsuarioEmail =
        this.especAplicacion.find(
          (e: any) => e.NOMBRE_OBJETO === 'EMAIL COMPRAS',
        )?.TITULO ?? '';
      this.especModificarOrden =
        this.especAplicacion.find(
          (e: any) => e.NOMBRE_OBJETO === 'MODIFICAR ORDEN',
        )?.ACTIVADO ?? false;
      this.especModoEnvioEmail =
        this.especAplicacion.find(
          (e: any) => e.NOMBRE_OBJETO === 'MODO ENVIO EMAIL',
        )?.VALOR_DEFECTO ?? '';
    }
  }
      ...
      .subscribe((resp) => {
        ...
        if (res[0].ErrMensaje !== '') {
          ...
          if (this.mnuAccion === 'new') {
            this.QFiltro =
              " ORDENES_COMPRA.ID_DOCUMENTO = '" +
              this.DOrdenCompra.ID_DOCUMENTO +
              "' " +
              " AND ORDENES_COMPRA.CONSECUTIVO = '" +
          ...
          // Inactiva cambios
          const objControl = [
            '__obj__ddAutorizaciones',
            '__obj__ddPermisosEspeciales',
            '__obj__ddUnidadNegocio',
            '__obj__ddConexiones',
          ];
          ...
          if (this.especModoEnvioEmail === 'AL GUARDAR') {
            const rep = this.listaReportes.find(
              (r) => r.especificacion.defectoEmail,
            );
            if (rep !== undefined) {
  ...
  opIrARegistro(accion: string): void {
    ...
    if (this.DOrdenCompra.ESTADO === 'ANULADO')
      this.prmUsrAplBarReg = {
        ...this.prmUsrAplBarReg,
        operacion: { r_modificar: this.especModificarOrden, r_eliminar: false },
      };
    else
      this.prmUsrAplBarReg = {
        ...this.prmUsrAplBarReg,
        operacion: { r_modificar: this.especModificarOrden, r_eliminar: true },
      };
    this._sbarreg.setObsMenuReg(this.prmUsrAplBarReg);

    // Consulta los otros datos: items, gravamenes, adicionales
    const prm = {
      ID_DOCUMENTO: this.DOrdenCompra.ID_DOCUMENTO,
      CONSECUTIVO: this.DOrdenCompra.CONSECUTIVO,
    };
    this._sdatos
      ...
      .subscribe((data: any) => {
        ...
        this.listaEmailsProveedor = [];

        // Actualizar correos al cargar una orden existente (si los proveedores ya cargaron)
        if (this.DProveedores) {
          const dnomprov = this.DProveedores.find(
            (p) => p.ID_PROVEEDOR == this.DOrdenCompra.ID_PROVEEDOR,
          );
          if (dnomprov) {
            this.listaEmailsProveedor = dnomprov.EMAILS || [];
            this.DOrdenCompra.EMAIL = this.listaEmailsProveedor
              .map((e) => e.EMAIL)
              .join(', ');
          }
        }
  ...
  opVista(): void {
    const VDatosRegAux = this.VDatosReg.map(
      ({
        ID_DOCUMENTO,
        PREFIJO,
        CONSECUTIVO,
  ...
  opBlanquearForma(): void {
    ...
    this.DGridSubTotales = [
      { key: 'Subtotal', value: 0 },
      { key: 'Descuentos', value: 0 },
      { key: 'Otros', value: 0 },
    ];
    this.gridBoxProveedor = [];
    if (this.UN_DEF != null) this.gridBoxUN = [this.UN_DEF];
    this.DOrdenCompra.ID_UN_DESTINO = this.UN_DEF;
    if (this.MONEDA_DEF != null) this.gridBoxMoneda = [this.MONEDA_DEF];
    ...
    this.DOrdenCompraItems_prev = JSON.parse(
      JSON.stringify(this.DOrdenCompraItems),
    );
    this.DOrdenCompraGrav_prev = JSON.parse(
      JSON.stringify(this.DOrdenCompraGrav),
    );
    this.TOT_GRAV = 0;
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            this.filasSelecc.forEach((key) => {
              const index = this.DOrdenCompraItems.findIndex(
                (a) => a.ITEM === key,
              );
            ...
            this.setItemValues();
            this.readOnlyForm =
              this.DOrdenCompraItems.length > 0 ? true : false;
          }
        });
        break;
  ...
  onChangeCantidad(rowData: any, value: any, currentRowData: any) {
    ...
    rowData.CANTIDAD_REAL =
      value * currentRowData.CANTIDAD_EQUIV * currentRowData.CANTIDAD_PRES;
    rowData.SUB_TOTAL = rowData.CANTIDAD * currentRowData.VALOR_UNITARIO;
    const porIva =
      currentRowData.GRAVAMENES.PORCENTAJE !== undefined
        ? currentRowData.GRAVAMENES.PORCENTAJE
        : 0;
  ...
  onCellHoverChanged(e: any) {
    if (
      e.rowType === 'data' &&
      e.column.dataField.match('PRODUCTO|NOMBRE_PRODUCTO')
    ) {
  ...
  onCellClick(e) {
    ...
    if (e.row.cells[e.columnIndex].column.dataField === 'VALOR_IVA') {
      ...
      const dnompro = this.DProductosLista.find(
        (p) => p.PRODUCTO == e.data.PRODUCTO,
      );
      if (dnompro) {
        if (dnompro.IVAS.length > 1) {
          ...
          if (e.data.GRAVAMENES) {
            if (e.data.GRAVAMENES.ID_TASA) ivaAso = e.data.GRAVAMENES.ID_TASA;
          }
          this.keyFila = e.key;
  ...
  onFocusOutAtributos(e, cellInfo) {
    if (cellInfo.data) {
      cellInfo.data.ATRIBUTO = Array.isArray(e.component.option('value'))
        ? ''
  ...
  onFocusOutClientes(e, cellInfo) {
    if (cellInfo.data) {
      cellInfo.data.ID_PROVEEDOR = Array.isArray(e.component.option('value'))
        ? ''
        : e.component.option('value');
    }
  }
  ...
  onFocusOutProductos(e, cellInfo) {
    if (cellInfo.data) {
      cellInfo.data.PRODUCTO = Array.isArray(e.component.option('value'))
        ? ''
        : e.component.option('value');
    }
  }
  ...
  onRowValidating(e: any) {
    ...
    for (let i = 0; i < this.DOrdenCompraItems.length; i++) {
      ...
      if (
        e.newData.PRODUCTO === ele.PRODUCTO &&
        e.newData.UDM_COMPRA === ele.UDM_COMPRA
      )
  ...
  onSeleccDocumento(e: any): void {
    ...
    if (this._sdatos.accion == 'r_nuevo' && this.DDocumentos) {
      const doc = this.DDocumentos.find((p) => p.DOCUMENTO === e.value);
      if (doc) {
        this.DOrdenCompra.ID_DOCUMENTO = doc.ID_DOCUMENTO;
        this.DOrdenCompra.CONSECUTIVO = doc.CONSECUTIVO;
      }
    }
  }
  ...
  onSelectionProveedor(e) {
    if (this.readOnly) return;
    if (e !== 'refrescar' && this.DProveedores) {
      const dnomprov = this.DProveedores.find((p) => p.ID_PROVEEDOR == e.value);
      if (dnomprov) {
        console.log('info del proveedor: ', dnomprov.EMAILS);
        ...
        this.DOrdenCompra.EMAIL = this.listaEmailsProveedor
          .map((e) => e.EMAIL)
          .join(', ');
        if (this.listaEmailsProveedor.length === 0)
          showToast('Proveedor no tiene correo electrónico!', 'error');

        console.log('Emails consolidados: ', this.DOrdenCompra.EMAIL);
        if (
  ...
  onSelectionCondicion(e) {
    ...
    if (e.value) {
      if (
        ...
        this.DOrdenCompraItems.length > 0
      ) {
        Swal.fire({
          title: '',
          text:
            'Está cambiando la Condición ' +
            this.DOrdenCompra.ID_CONDICION +
            ' por ' +
            e.value +
            ', esto borrará los Productos asociados, ¿Desea Continuar?',
          iconHtml: "<i class='icon-alert-ol'></i>",
          showCancelButton: true,
          confirmButtonColor: '#DF3E3E',
          cancelButtonColor: '#438ef1',
      ...
      this.DOrdenCompra.ID_CONDICION = e.value;
      const dplazos = this.DCondiciones.find(
        (p) => p.ID_CONDICION == e.value,
      ).PLAZO;
      if (dplazos) {
  ...
  onSeleccRowAtributo(e, cellInfo, datacompo) {
    if (e.data) {
      ...
      this.gridOrdenCompraItems.instance.cellValue(
        cellInfo.rowIndex,
        'ATRIBUTO',
        cellInfo.data.ATRIBUTO,
      );
      datacompo.close();
      this.isGridBoxAtrOpened = false;
    }
  ...
  onSeleccRowCliente(e, cellInfo, datacompo) {
    if (e.data) {
      ...
      this.DOrdenCompra.NOMBRE_PROVEEDOR = this.DProveedores.find(
        (p) => p.ID_PROVEEDOR === cellInfo.data.ID_PROVEEDOR,
      ).NOMBRE_COMPLETO;
  ...
  async onSeleccRowProducto(e: any, cellInfo: any) {
    if (e.selectedRowKeys[0]) {
      if (
        this.DOrdenCompraItems.findIndex(
          (d: any) => d.PRODUCTO === e.selectedRowKeys[0],
        ) === -1
      ) {
        ...
        if (dnompro) {
          ...
          if (dnompro.PRESENTACION) {
            ...
            if (dpres.length === 1) {
              ...
              for (let i = 0; i < this.DOrdenCompraItems.length; i++) {
                const ele = this.DOrdenCompraItems[i];
                if (
                  cellInfo.data.PRODUCTO === ele.PRODUCTO &&
                  dpres[0].UDM_COMPRA === ele.UDM_COMPRA
                ) {
                  showToast(
                    'Producto con Unidad de Compra repetido.',
                    'warning',
                  );
                  return;
                }
          ...
          setTimeout(() => {
            ...
            // e.component.focus();
            cellInfo.component.focus(
              this.gridOrdenCompraItems.instance.getCellElement(
                rowIndex,
                'PRODUCTO',
              ),
            );
  ...
  customizeColumnsSearch(columns: Array<any>) {
    columns.forEach((column: any) => {
      column.calculateFilterExpression =
        this.ltool.customCalculateFilterExpression.bind(column);
  ...
  onDropdownOpenedPro(e) {
    setTimeout(() => {
      if (this.gridProLista?.instance) {
        ...
        const searchInput =
          gridElement.querySelector('.dx-datagrid-search-panel input') ||
  ...
  onSoloProPrecio(e) {
    ...
    if (e.value) {
      ...
      if (!chkprov)
        this.gridProLista.instance.filter((p) => {
          return p.PRECIO != 0;
        });
      else
        this.gridProLista.instance.filter((p) => {
          return (
            p.PRECIO != 0 && p.PROVEEDOR.match(this.DOrdenCompra.ID_PROVEEDOR)
          );
        });
    } else {
      // datPro = !chkprov ?
      ...
      if (!chkprov) this.gridProLista.instance.filter(null);
      else
        this.gridProLista.instance.filter((p) => {
          return p.PROVEEDOR.match(this.DOrdenCompra.ID_PROVEEDOR);
        });
    }
  ...
  onSoloProProveedor(e) {
    ...
    if (e.value) {
      datPro = !chkprecio
        ? this.DProductosLista.filter((p) =>
            p.PROVEEDOR.match(this.DOrdenCompra.ID_PROVEEDOR),
          )
        : this.DProductosLista.filter(
            (p) =>
              p.PROVEEDOR.match(this.DOrdenCompra.ID_PROVEEDOR) &&
              p.PRECIO !== 0,
          );
  ...
  onSeleccIvas(e) {
    ...
    var rowIndex = this.gridOrdenCompraItems.instance.getRowIndexByKey(
      this.keyFila,
    );
    const canPro = this.gridOrdenCompraItems.instance.cellValue(
      rowIndex,
      'CANTIDAD',
    );
    const valUnit = this.gridOrdenCompraItems.instance.cellValue(
      rowIndex,
      'VALOR_UNITARIO',
    );
    ...
    const nx = this.DOrdenCompraItems.findIndex(
      (it) =>
        it.ITEM ==
        this.gridOrdenCompraItems.instance.cellValue(rowIndex, 'ITEM'),
    );
    if (nx !== -1) {
  ...
  onValueChangedUMCompra(e, cellInfo) {
    ...
    if (
      this.DOrdenCompraItems.findIndex(
        (d: any) =>
          d.PRODUCTO === cellInfo.data.PRODUCTO && d.UDM_COMPRA === e.value,
      ) === -1
    ) {
      if (dum) {
        ...
        cellInfo.data.CODIGO_ASOCIADO = dum.CODIGO_ASOCIADO;
        const dcanreal =
          dum.CANTIDAD_PRES * dum.CANTIDAD_EQUIV * cellInfo.data.CANTIDAD;
  ...
  clickGravamenes(e, accion: string) {
    if (accion === 'apl grav') {
      ...
      Swal.fire({
        title: '',
        text:
          'Se asignarán los gravamenes a la transacción que se seleccionen. ' +
        ...
        if (result.isConfirmed) {
          ...
          this.eventsSubjectGridTasas.next({
            ...
            rowsSelected: this.DOrdenCompraGrav.map(({ ID_TASA }) => ({
              ID_TASA,
            })),
            keyExpr: ['ID_TASA'],
      ...
      Swal.fire({
        title: '',
        text:
          'Se asignarán los gravámenes asociados a los productos y del proveedor. ' +
          'Desea proceder?',
        iconHtml: "<i class='icon-alert-ol'></i>",
        showCancelButton: true,
        confirmButtonColor: '#DF3E3E',
  ...
  AccionEliminar(): void {
    ...
    const prm = {
      ORDEN_COMPRA: {
        ID_DOCUMENTO: this.DOrdenCompra.ID_DOCUMENTO,
        PREFIJO: this.DOrdenCompra.PREFIJO,
        CONSECUTIVO: this.DOrdenCompra.CONSECUTIVO,
        SUFIJO: this.DOrdenCompra.SUFIJO,
      },
    };
    this._sdatos
  ...
  calcularValores() {
    this.DOrdenCompra.SUB_TOTAL = this.DOrdenCompraItems.reduce(
      (n, { SUB_TOTAL }) => n + SUB_TOTAL,
      0,
    );
    if (this.DOrdenCompraGrav !== undefined)
      this.TOT_GRAV = this.DOrdenCompraGrav.reduce(
        (n, { VALOR }) => n + VALOR,
        0,
      );
    ...
    this.DGridSubTotales = [
      { key: 'Subtotal', value: this.DOrdenCompra.SUB_TOTAL },
  ...
  imprimirReporte(operimp) {
    let filtroRep: any = '';
    if (operimp.registro === 'todos') filtroRep = { FILTRO: this.QFiltro };
    else
      filtroRep = {
        FILTRO:
          " ORDENES_COMPRA.ID_DOCUMENTO = '" +
    ...
    const listaTab = this.tabService.tabs.find(
      (c) => c.aplicacion === operimp.id_reporte,
    );
    if (listaTab === undefined)
      GlobalVariables.listaAplicaciones.unshift({
        aplicacion: operimp.id_reporte,
        barra: undefined,
        statusEdicion: '',
      });
  ...
  enviarEmailPDF() {
    ...
    if (!this.DProveedores) {
      showToast(
        'Cargando datos de proveedores, reintente en un momento...',
        'warning',
      );
      return;
    }

    const dprov = this.DProveedores.find(
      (p) => p.ID_PROVEEDOR === this.DOrdenCompra.ID_PROVEEDOR,
    );
    if (!dprov) {
    ...
    console.log('Datos para envío de email:', dprov);
    let asunto =
      this.especAplicacion.find((e: any) => e.NOMBRE_OBJETO === 'ASUNTO EMAIL')
        ?.VALOR_DEFECTO ?? '';
    let email_sale =
      this.especAplicacion.find(
        (e: any) => e.NOMBRE_OBJETO === 'EMAIL SALIENTE',
      )?.VALOR_DEFECTO ?? '';
    ...
    const dataSource = {
      ...
      DESTINO: dprov ? dprov.NOMBRE_COMPLETO : '',
      DESTINO_EMAIL: this.DOrdenCompra.EMAIL,
      ASUNTO: asunto,
      NOTA: '',
    };
  ...
  onRespuestaEnvioCorreo(e: any) {
    // console.log('Respuesta del envío de correo: ', e);
    ...
    const ext = this.ltool.makeRandom(30);
    let filtroRep: any = '';
    if (this.rpt_registro === 'todos') filtroRep = { FILTRO: this.QFiltro };
    else
      filtroRep = {
        FILTRO:
          " ORDENES_COMPRA.ID_DOCUMENTO = '" +
    ...
    const prmRpt = {
      ...
      replacements: {
        ...
        DIA_ENTREGA: this.datepipe.transform(
          this.DOrdenCompra.FECHA_ENTREGA,
          'MMMM d, y',
          undefined,
          'es',
    ...
    console.log('prmRpt: '+ JSON.stringify(prmRpt));
    ...
    this._sgenerales.ExportarPDF(prmRpt).subscribe((data: Blob) => {
      ...
      this._sgenerales
        ...
        .subscribe((data: any) => {
          const res = JSON.parse(data.data);
          if (res[0].ErrMensaje !== '') showToast(res[0].ErrMensaje);
        });
      let datos = {
        data: {
          datos: prmRpt,
      ...
      console.log('Respuesta del backend ? '+JSON.stringify(datos));
  ...
  initNewRow(e) {
    ...
    if (this.DOrdenCompraItems.length > 0) {
      const item = this.DOrdenCompraItems.reduce((ant, act) => {
  ...
  setDocumentoToDatatable() {
    this.DDocumentos = [];
    this.DDocumentos.push({
      ID_DOCUMENTO: this.DOrdenCompra.ID_DOCUMENTO,
      CONSECUTIVO: this.DOrdenCompra.CONSECUTIVO,
      DOCUMENTO:
        this.DOrdenCompra.ID_DOCUMENTO + '-' + this.DOrdenCompra.CONSECUTIVO,
    });
  }
  ...
  getCodigoAsociado (rowData) {
    return rowData.CODIGO_ASOCIADO || '';
  };
  ...
  ValidaDatos(Accion: string): boolean {
    ...
    if (Accion === 'requerido') {
      ...
      if (this.DOrdenCompra.FECHA_SOLICITUD == null)
        msg += 'Fecha de solicitud,';
      if (
        this.DOrdenCompra.FECHA_ENTREGA === null ||
        this.DOrdenCompra.FECHA_ENTREGA === ''
      )
      ...
      if (
        this.DOrdenCompraItems === undefined ||
        this.DOrdenCompraItems.length === 0
      )
        msg +=
          'Productos: No se ha asociado ningún Producto a la Orden de compra.';
      if (msg !== '') {
        this.showModal(
          'Hay datos incompletos de la Orden de compra. Revisar contenido de los items: ' +
            msg,
          'Faltan datos',
          '',
        );
  ...
  valoresObjetos(obj: string, opcion: any = undefined) {
    if (obj == 'documento' || obj == 'todos') {
      ...
      this._sdatos
        ...
        .subscribe((data: any) => {
          ...
          if (
            this.DDocumentos != null &&
            this.DDocumentos.length == 1 &&
            !this.readOnly
          ) {
    ...
    if (obj == 'proveedores' || obj == 'todos') {
      this._sdatos
        .consulta(
          'proveedores',
          { ESTADO: 'ACTIVO', CONSULTA: 'compras' },
          'CXP-200',
        )
        .subscribe((data: any) => {
          const res = JSON.parse(data.data);
          ...
          // Si ya hay un proveedor seleccionado (carga inicial), refrescar correos
          if (this.DOrdenCompra.ID_PROVEEDOR) {
            const dnomprov = this.DProveedores.find(
              (p) => p.ID_PROVEEDOR == this.DOrdenCompra.ID_PROVEEDOR,
            );
            if (dnomprov) {
          ...
          this.eventsSubjectSboxProv.next({
            dataSource: this.DProveedores.map(
              ({ ID_PROVEEDOR, NOMBRE_COMPLETO }) => ({
                ID_PROVEEDOR,
                NOMBRE_COMPLETO,
              }),
            ),
          ...
          console.log('DProveedores', this.DProveedores);
        });
    }
          ...
          if (res[0].ErrMensaje !== '') {
            ...
            if (opcion !== undefined) {
              this.DOrdenCompraItems.forEach((item) => {
                const nx = this.DOrdenCompraGravItems.findIndex(
                  (gi) => gi.ITEM == item.ITEM,
                );
    ...
    if (obj === 'tasas legales' || obj === 'todos') {
      const prm = {};
      this._sdatos
        .consulta('LISTA GRAVAMENES', prm, 'generales')
        .subscribe((data: any) => {
          const res = validatorRes(data);
          if (res[0].ErrMensaje !== '') {
            this.showModal(res[0].ErrMensaje);
          } else this.DGravamenes = res;
          this.DGravamenes.forEach((eleg) => {
            eleg.PORCENTAJE =
              (eleg.PORCENTAJE < 0 ? '(' : '') +
              formatNumber(
                (eleg.PORCENTAJE < 0 ? -1 : 1) * eleg.PORCENTAJE * 100,
                'en-US',
                '1.2-2',
              ) +
              (eleg.PORCENTAJE < 0 ? ')' : '');
    ...
    if (obj == 'monedas' || obj == 'todos') {
      this._sdatos
        .consulta('MONEDAS', { ESTADO: 'ACTIVO' }, 'ADM-004')
        .subscribe((data: any) => {
          const res = JSON.parse(data.data);
          if (data.token != undefined) {
            const refreshToken = data.token;
    ...
    if (obj == 'condicion' || obj == 'ref') {
      this._sdatos
        ...
        .subscribe((data: any) => {
          ...
          this.DPlazos = this.DCondiciones.find(
            (e) => e.ID_CONDICION === this.DOrdenCompra.ID_CONDICION,
          ).PLAZO;
          this.eventsSubjectSboxCond.next({
            dataSource: this.DCondiciones,
            valueExpr: 'ID_CONDICION',
      ...
      this._sbarreg.listaInformes(prm).subscribe((data) => {
        ...
        res.forEach((rep: any) => {
          this.listaReportes.push({
            ...
            item: k,
            especificacion:
              rep.ESPECIFICACION !== '' ? JSON.parse(rep.ESPECIFICACION) : {},
          });
          k++;
        });
      });
      ...
      this._sdatos
        ...
        .subscribe((data: any) => {
          ...
          if (this.logEmailNoEnviados.length !== 0) {
            ...
            this.showModal(
              '',
              'No se han enviado email para algunas Ordenes de compra:',
              htmmsg,
            );
          }
        });
