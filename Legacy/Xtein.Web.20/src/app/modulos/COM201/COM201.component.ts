    ...
    if (obj == 'generar precios grupo') {
      ...
      this._sdatos.consulta('generar precios grupo',prm,this.prmUsrAplBarReg.aplicacion).subscribe((data: any)=> {
        ...
        if (res[0].ErrMensaje !== '') {
          ...
          // Obtener todos los grupos afectados (grupo actual + subgrupos)
          this.filtroGrupo = [];
          this.listaGruposRecur(this.grupoSelecc);

          // Filtrar la respuesta para obtener solo productos del grupo seleccionado
          const productosGrupo = res.filter(item => this.filtroGrupo.includes(item.ID_GRUPO));

          // Crear un Set con los productos que vienen en la respuesta (para búsqueda rápida)
          const productosRespuesta = new Set(productosGrupo.map(p => p.PRODUCTO));

          // Eliminar de DItemsLista los productos del grupo que YA NO están en la respuesta
          this.DItemsLista = this.DItemsLista.filter(item => {
            // Si el item NO es del grupo afectado, mantenerlo
            if (!this.filtroGrupo.includes(item.ID_GRUPO)) return true;
            // Si es del grupo, solo mantenerlo si viene en la respuesta
            return productosRespuesta.has(item.PRODUCTO);
          });

          // Actualizar/agregar productos del grupo que vienen en la respuesta
          productosGrupo.forEach(lis => {
            const ix = this.DItemsLista.findIndex(l => l.PRODUCTO === lis.PRODUCTO);
            if (ix !== -1) {
              // Actualizar producto existente
              this.DItemsLista[ix] = lis;
            } else {
              // Agregar producto nuevo
              this.DItemsLista.push(lis);
            }
          });
