...
import { COM20206Component } from './../COM20206/COM20206.component';
...
export class COM20202Component {
  ...
  gridOrdenCompra: DxDataGridComponent;
  ...
  constructor(
    ...
    // Suscripción a eventos de barra de registro
  ...
  opMenuRegistro(operMenu: clsBarraRegistro): void {
    ...
    switch (operMenu.accion) {
      ...
      case 'r_refrescar':
        ...
        // Refrescar grid si tiene filtro aplicado
        if (this.filtroDefecto) {
  ...
  puedeReprogramar(data: any): boolean {
    if (!data || !data.ESTADO) return true;
    return !data.ESTADO.match('ANULADO|CERRADA|TERMINADA');
  }

  abrirSeguimiento(data: any) {
    if (!this.puedeReprogramar(data)) return;

    // Completa la cantidad pendiente de cada item de ITEMS con lo que ya tenemos cargado en la grid
    const dataEnviada = { ...data };
    if (dataEnviada.ITEMS) {
      const items = JSON.parse(dataEnviada.ITEMS);
      dataEnviada.ITEMS = JSON.stringify(
        items.map((it: any) => {
          const itemGrid = this.DOrdenCompraItems.find(
            (o) => o.DOCUMENTO === data.DOCUMENTO && o.PRODUCTO === it.PRODUCTO,
          );
          return { ...it, CANTIDAD_PENDIENTE: itemGrid ? itemGrid.CANTIDAD_PENDIENTE : undefined };
        }),
      );
    }
  ...
  onRespuestaSeg(e: any) {
    // Refresca la grid para reflejar la nueva fecha reprogramada del item
    this.valoresObjetos('ordenes de compra');
  }
      ...
      e.items = [
        {
          text: 'Reprogramar',
          icon: 'event',
          disabled: !this.puedeReprogramar(e.row.data),
          onClick: () => {
            this.abrirSeguimiento(e.row.data);
          },
        },
      ];
    }
  ...
  esFechaVencida(fecha: Date, fecha_entrega: Date): boolean {
    if (!fecha) return false;

    const compare_date = new Date(fecha_entrega);
    compare_date.setHours(0, 0, 0, 0); // Limpiamos horas para comparar solo días

    const fechaRepro = new Date(fecha);
    fechaRepro.setHours(0, 0, 0, 0);

    // Retorna true si la fecha de reprogramación es mayor a la fecha de entrega
    return fechaRepro > compare_date;
  }
  ...
  onCellClick(e) {
    ...
    if (e.row.cells[e.columnIndex].column.dataField === 'DOCUMENTO') {
      ...
      const compoApl = {
        ...
        FILTRO:
          '{ "ESTRUCTURA": ' +
          '[{"CAMPO": "ID_DOCUMENTO", "EXPRESION": "' +
          e.row.data.ID_DOCUMENTO +
          '"},' +
          ' {"CAMPO": "CONSECUTIVO", "EXPRESION": "' +
          e.row.data.CONSECUTIVO +
          '"}] }',
      };
    ...
    if (e.data) {
      ...
      this._sdatos
        ...
        .subscribe((resp) => {
          ...
          const ix = this.DOrdenCompraItems.findIndex(
            (o) => o.ITEM == e.data.ITEM,
          );
          if (res[0].ErrMensaje !== '') {
            ...
            showToast(
              'Cantidad actualizada. Revisar orden de compra',
              'success',
            );
          }
        });
  ...
  valoresDefecto() {
    if (this.especAplicacion) {
      this.especMoneda =
        this.especAplicacion.find((e) => e.NOMBRE_OBJETO === 'ID_MONEDA_DEF')
          ?.VALOR_DEFECTO ?? '';
      this.especMonedaFormato =
        this.especAplicacion.find((e) => e.NOMBRE_OBJETO === 'FORMATO MONEDA')
          ?.FORMATO ?? '#,##0.00';
      this.especCantidadFormato =
        this.especAplicacion.find((e) => e.NOMBRE_OBJETO === 'FORMATO CANTIDAD')
          ?.FORMATO ?? '#,##0.00';
      this.especModificarCantidad =
        this.especAplicacion.find(
          (e) => e.NOMBRE_OBJETO === 'MODIFICAR CANTIDAD',
        )?.ACTIVADO ?? false;
      this.especModificarGrav =
        this.especAplicacion.find(
          (e) => e.NOMBRE_OBJETO === 'MODIFICAR GRAVAMENES',
        )?.ACTIVADO ?? false;
      this.especRepetirItem =
        this.especAplicacion.find((e) => e.NOMBRE_OBJETO === 'REPETIR ITEM')
          ?.ACTIVADO ?? false;
      this.especOrigenPrecio =
        this.especAplicacion.find((e) => e.NOMBRE_OBJETO === 'MODIFICAR PRECIO')
          ?.VALOR_DEFECTO ?? '';

      // Cuentas de email usuario
      this.especEmailCompras =
        this.especAplicacion.find((e) => e.NOMBRE_OBJETO === 'EMAIL COMPRAS')
          ?.VALOR_DEFECTO ?? '';
      this.especUsuarioEmail =
        this.especAplicacion.find((e) => e.NOMBRE_OBJETO === 'EMAIL COMPRAS')
          ?.TITULO ?? '';
      this.especModificarOrden =
        this.especAplicacion.find((e) => e.NOMBRE_OBJETO === 'MODIFICAR ORDEN')
          ?.ACTIVADO ?? false;
      if (this.especModificarCantidad) this.esEdicion = true;
    }
  }
  ...
  onSeleccAcciones(e) {
    ...
    const documento = this.gridOrdenCompra.instance.cellValue(
      rowIndex,
      'DOCUMENTO',
    );
    ...
    this._sdatos
      ...
      .subscribe((data: any) => {
        ...
        if (res[0].ErrMensaje !== '') {
          ...
          // Actualiza los datos
          const dord = this.DOrdenCompraItems.filter(
            (oc) => oc.DOCUMENTO === documento,
          );
          if (dord) {
            dord.forEach((ord) => {
  ...
  valoresObjetos(obj: string, opcion: any = undefined) {
    ...
    if (obj == 'ordenes de compra') {
      ...
      this._sdatos
        .consulta(
          'GESTION DETALLES ORDENES COMPRA',
          {
            ID_APLICACION: this.prmUsrAplBarReg.aplicacion,
            USUARIO: this.prmUsrAplBarReg.usuario,
    ...
    if (obj == 'filtros gestion de compras' || obj == 'todos') {
      this._sdatos
        .consulta(
          'FILTROS GESTION DETALLES ORDENES COMPRA',
          {
            ID_APLICACION: this.prmUsrAplBarReg.aplicacion,
            USUARIO: this.prmUsrAplBarReg.usuario,
          },
  ...
  // Usamos funciones flecha (=>) para no perder el contexto del 'this'
  saveGridState = (state: any) => {
    if (state) {
      // 1. Clonamos profundamente el objeto para NO romper ni congelar la Grid en vivo
      const stateToSave = JSON.parse(JSON.stringify(state));

      // 2. Reseteamos el buscador global y filtros generales del panel
      stateToSave.filterValue = null;
      stateToSave.searchText = '';

      // 3. Limpiamos los filtros internos de cada columna (dejando intacto el diseño/ancho/visibilidad)
      if (stateToSave.columns) {
        stateToSave.columns.forEach((col: any) => {
          col.filterValue = null;
          col.filterValues = null; // Crucial: Esto limpia los Checkboxes del Header Filter
          col.selectedFilterOperation = null;
        });
      }

      // 4. Guardamos en el localStorage la configuración limpia de estructura
      localStorage.setItem('com20202-grid-state', JSON.stringify(stateToSave));
    }
  };

  loadGridState = () => {
    // Leemos la estructura limpia del localStorage cuando la grid se inicializa
    const savedState = localStorage.getItem('com20202-grid-state');
  ...
  ngOnInit(): void {
    ...
    this._sbarreg.carguemenu(this.prmUsrAplBarReg).subscribe((data: any) => {
      const res = JSON.parse(data.data);
      this.r_modificar = res.r_modificar;
    });
  ...
  ngOnDestroy() {
    ...
    // Eliminar estado guardado para que los filtros no persistan al reabrir el módulo
    // localStorage.removeItem('com20202-grid-state');
    // 1. Limpieza de filtros en la instancia viva de la Grid antes de salir
    if (this.gridOrdenCompra && this.gridOrdenCompra.instance) {
      this.gridOrdenCompra.instance.clearFilter();
    }
  }
