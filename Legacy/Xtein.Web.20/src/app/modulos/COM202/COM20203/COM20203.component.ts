...
export class COM20203Component {
  ...
  constructor(
    ...
    // Inicializa la suscripción a eventos de barra de registro
  ...
  enviarEmailPDF() {
    ...
    if (!this.DProveedores) {
      showToast('Cargando datos de proveedores, reintente en un momento...', 'warning');
      return;
    }

    var rowIndex = this.gridOrdenCompra.instance.getRowIndexByKey(this.keyFila);
    const rowData = this.gridOrdenCompra.instance.getVisibleRows()[rowIndex].data;
    const prov = rowData.ID_PROVEEDOR;
    const doc = rowData.DOCUMENTO;
    ...
    console.log("Datos para envío de email (proveedor):", dprov);
    console.log("Datos de la fila OC:", rowData);
    ...
    const dataSource = {
      ...
      DESTINO_EMAIL: (dprov && dprov.EMAILS && Array.isArray(dprov.EMAILS))
        ? dprov.EMAILS.map((e: any) => e.EMAIL).join(', ')
        : (dprov ? (dprov.EMAILS || dprov.EMAIL) : ''),
      ASUNTO: asunto,
      NOTA: ''
    }
    console.log("especAplicacion", this.especAplicacion)
    console.log("datasource para modal", dataSource);
  ...
  ngOnDestroy() {
    ...
    if (this.gridOrdenCompra?.instance) {
      this.gridOrdenCompra.instance.clearFilter();
      this.gridOrdenCompra.instance.searchByText('');
    }
  }
