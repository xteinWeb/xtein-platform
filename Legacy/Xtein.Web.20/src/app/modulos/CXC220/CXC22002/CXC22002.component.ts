...
export class CXC22002Component {
  ...
  eventsSubscriptionSolicitud: Subscription;
  ...
  constructor(
    ...
    this.calcularLiquidacion = this.calcularLiquidacion.bind(this);
  ...
  onSolicitudSeleccionada(e: any): void {
    if (e.selectedItem) {
      ...
      const docSol = {
        DOCUMENTO: e.selectedItem.DOCUMENTO,
  ...
  onDatosCambio(): void {
    ...
    if (`${item.NOMBRE1 || ''} ${item.NOMBRE2 || ''} ${item.APELLIDO1 || ''} ${item.APELLIDO2 || ''}`.trim() !== '') {
      ...
      this.listaAsociados[this.nItemSolicitud].id = this.nItemSolicitud;
      this.listaAsociados[this.nItemSolicitud].asociado = item.ID_CLIENTE ? `${item.ID_CLIENTE}` : `${calidad}`;
  ...
  customizeColumnsFin(columns) {
    ...
    this.gridFinanciacion.instance.refresh();
  ...
  valoresObjetos(obj: string, opcion: any = undefined) {
    ...
    if (obj === 'productos' || obj === 'ref') {
      const prm = {
        ID_LISTA: this.solicitud.ID_CONDICION,
        TIPO: "CONDICION",
        PLAZO: 1,
        CON_EXIS: "NO",
        ID_UN_BODEGA: ''
    ...
    if (obj === 'cargar solicitud' || obj === 'ref') {
      ...
      this._sdatos.consulta('solicitud credito', prm, 'CXC-220').subscribe((data: any) => {
        ...
        this.itemSolicitud = res.ITEMS_SOLICITUD;
        this.fianzasSeguros = res.FIANZAS;
        this.datosFinanciacion = (res.TABLA_FIN[0]?.PARAMETROS ? res.TABLA_FIN[0].PARAMETROS : res.TABLA_FIN);
  ...
  ngOnInit(): void {
    ...
    // Si llega una petición de consulta
    this.eventsSubscriptionSolicitud = this.events.subscribe((datos: any) => {
      if (datos.accion === 'consulta') {
        this.valoresObjetos('cargar solicitud', datos.dato);
      }
    });
  ...
  ngOnDestroy() {
    if (this.eventsSubscriptionSolicitud) {
      this.eventsSubscriptionSolicitud.unsubscribe();
    }
  ...
  onTagAdded(tagData: any): void {
    ...
    if (tagData.accion === 'seleccionar') {
      this.nItemSolicitud = tagData.dato.id;
      notify('Se seleccionó el asociado ' + tagData.dato.name, 'success', 2000);
    }
