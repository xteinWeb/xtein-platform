...
export class FIN221Component {
  ...
  //     setTimeout(() => {
  //       var fNextEditor: any = this.formPlanesFin.instance.getEditor('ID_PLAN');
  //       if (fNextEditor) {
  //         fNextEditor.focus();
  //       }
  //     }, 100);
  ...
  ValideExistenciaPlan = async (e: any, showWarning = true): Promise<boolean> => {
    if (this.readOnly || !this.mnuAccion.match('new|update')) {
      return true;
    }
    ...
    const valor = e.value.toString().trim().toUpperCase();

    if (
      this.mnuAccion === 'update' &&
      this.FPlanes_prev &&
      this.FPlanes_prev.ID_PLAN?.toString().trim().toUpperCase() === valor
    ) {
      return true;
    }

    try {
      const prm = { ID_PLAN: valor };
      const apiRest = this.sData.consulta(
        'existe',
        prm,
        this.prmUsrAplBarReg.aplicacion
      );
      let res = await lastValueFrom(apiRest, { defaultValue: null });
      if (!res || !res.data) return true;

      res = JSON.parse(res.data);
      if (res && res[0] && res[0].ErrMensaje === '') {
        return true;
      } else {
        if (showWarning) {
          this.showModal(
            `El plan con código "${e.value}" ya está registrado en el sistema. No se puede usar este código.`,
            'Plan ya existe'
          );
          // Enfocar el campo ID_PLAN
          setTimeout(() => {
      ...
      console.error('Error al validar existencia de plan:', error);
      return true;
    }
  };

  onValueChangedIdPlan = ($event: any) => {
    this.onValueChanged($event, 'ID_PLAN');

    // 2. Forzamos a DevExtreme a validar SOLO este campo inmediatamente en pantalla
    if ($event.value) {
      setTimeout(async () => {
        // Esto obliga al validador del campo ID_PLAN a ejecutarse y pintar la línea roja si falla
        const result = await this.formPlanesFin.instance.validate();
      }, 150); // Le damos un mini respiro para que el bindeo se actualice
    }
  };
  ...
  opIrARegistro(accion: string): void {
    ...
    switch (accion) {
      ...
      case 'Eliminado':
        ...
        if (this.VDatosReg.length >= 0) {
          this.FPlanes = this.VDatosReg[this.prmUsrAplBarReg.r_numReg - 1];
        }
    ...
    try {
      ...
      if (this.Itm_PlanesFin.length > 0) {
        for (let i = 0; i < this.Itm_PlanesFin.length; i++) {
          ...
          element.CUOTA_INICIAL === true
            ? (element.CUOTA_INICIAL = 'Si')
            : (element.CUOTA_INICIAL = 'No');
          element.DIFERIR_CI === true
            ? (element.DIFERIR_CI = 'Si')
            : (element.DIFERIR_CI = 'No');
          element.DESCONTAR_CI === true
            ? (element.DESCONTAR_CI = 'Si')
            : (element.DESCONTAR_CI = 'No');
        }
        // ...existing code...
      }
      this.readOnly = true;
  ...
  ValidaDatos(Accion: string): boolean {
    var msg = '';
    if (Accion === 'requerido') {
      if (!this.FPlanes.ID_PLAN) msg += 'Id Plan, ';
      if (!this.FPlanes.DESCRIPCION) msg += 'Descripción, ';
      if (!this.FPlanes.TIPO_FRECUENCIA) msg += 'Frecuencia, ';
      if (!this.FPlanes.ESTADO) msg += 'Estado, ';

      // Si es tipo Constante, exigir número de pagos y días
      if (this.FPlanes.TIPO_FRECUENCIA === 'Constante') {
        if (!this.FPlanes.FRECUENCIA || this.FPlanes.FRECUENCIA <= 0)
          msg += 'N° Días (Frecuencia), ';
      }

      if (!this.FPlanes.NUMERO_PAGOS || this.FPlanes.NUMERO_PAGOS <= 0)
        msg += 'Número de Pagos, ';

      if (msg !== '') {
        this.showModal(
          'Hay datos incompletos en el Plan Financiero: ' +
  ...
  async opPrepararGuardar(): Promise<void> {
    // Acción validación de datos

    // 1. Ejecutar la validación nativa
    const validationResult = this.formPlanesFin.instance.validate();

    // 2. Si la regla asíncrona está pendiente, esperamos y capturamos el resultado final
    if (validationResult.status === 'pending') {
      const asyncResult = await validationResult.complete;
      if (!asyncResult!.isValid) return;
    ...
    // Forzar al Grid a guardar cualquier fila que se esté editando en este instante
    if (this.gridPlanesFin && this.gridPlanesFin.instance.hasEditData()) {
      await this.gridPlanesFin.instance.saveEditData();
    }

    // OBTENER VALOR REAL DEL EDITOR: Asegura evaluar el texto que está visualizándose en pantalla
    const idPlanRaw =
      this.formPlanesFin.instance.getEditor('ID_PLAN')?.option('value') ||
      this.FPlanes.ID_PLAN || '';
    const idPlanActual = idPlanRaw.toString().trim().toUpperCase();

    // Validación de duplicación de ID_PLAN (chequeo manual como red de seguridad contra la base de datos)
    const esValido = await this.ValideExistenciaPlan({ value: idPlanActual }, false);
    if (!esValido) {
      this.showModal(
        `El plan con código "${idPlanRaw.toString().trim()}" ya existe. Por favor ingrese un código diferente o mantenga el original.`,
        'Plan duplicado',
      );
      return;
    }

    // Aseguramos que el objeto final que se va a guardar lleve el valor correcto antes de armar el JSON
    this.FPlanes.ID_PLAN = idPlanActual;
    ...
    if (this.conCambios > 0) {
      ...
      for (let i = 0; i < this.Itm_PlanesFin.length; i++) {
        ...
        element.CUOTA_INICIAL === 'Si'
          ? (element.CUOTA_INICIAL = true)
          : (element.CUOTA_INICIAL = false);
        element.DIFERIR_CI === 'Si'
          ? (element.DIFERIR_CI = true)
          : (element.DIFERIR_CI = false);
        element.DESCONTAR_CI === 'Si'
          ? (element.DESCONTAR_CI = true)
          : (element.DESCONTAR_CI = false);
      }
      const prmDatos: any = {
        PLANES: this.FPlanes,
      ...
      this.sData
        .save(this.mnuAccion, prmDatos, this.prmUsrAplBarReg.aplicacion)
        .subscribe((resp) => {
          this.loadingVisible = false;
  ...
  opEliminar(): void {
    Swal.fire({
      title: '',
      text: '',
      html: 'Desea eliminar el PLAN <i>' + this.FPlanes.ID_PLAN + '</i> ?',
      iconHtml: "<i class='icon-alert-ol'></i>",
      showCancelButton: true,
      confirmButtonColor: '#DF3E3E',
      cancelButtonColor: '#438ef1',
    ...
    if (
      ...
      if (this.FPlanes.TIPO_FRECUENCIA === 'Variable') {
        const initItems = () => {
          ...
          // Obtenemos una condición por defecto segura para evitar el nulo en BD
          const condicionPorDefecto =
            this.DCondiciones && this.DCondiciones.length > 0
              ? this.DCondiciones[0].ID_CONDICION
              : ''; // O pon aquí el código de la condición por defecto si te lo sabes de memoria (ej: 'CONTADO')

          for (let i = 0; i < $event.value; i++) {
            this.Itm_PlanesFin.push({
              ITEM: i + 1,
              PAGO: i + 1,
              DIAS: 30,
              PORC_GRAV: 0,
              ...
              // =======================================================
              // BLINDAJE CONTRA NULOS AJUSTADO A TU CONTEXTO
              // =======================================================
              ID_CONDICION: condicionPorDefecto, // ¡Adiós nulos! Toma la primera condición automáticamente
              ID_TASA: '', // Correcto que vaya vacío en Variable
              PORC_TASA: 0,
              FACTOR: 0,
              CUOTA_INICIAL: 'No', // Conservamos tus strings originales
              DIFERIR_CI: 'No',
              DESCONTAR_CI: 'No',
            });
  ...
  valoresObjetos(obj: string) {
    if (obj === 'tasas' || obj === 'todos') {
      const prm = {};
      this.sData
        .consulta('lista tasas', prm, this.prmUsrAplBarReg.aplicacion)
        .subscribe((data: any) => {
          const res = JSON.parse(data.data);
          if (data.token != undefined) {
            const refreshToken = data.token;
  ...
  async getFactor(idTasa: any, pago: any): Promise<number> {
    ...
    return new Promise<number>((resolve) => {
      this.sData
        .consulta('factor', prm, this.prmUsrAplBarReg.aplicacion)
        .subscribe(
          (data: any) => {
            const res = JSON.parse(data.data);
            if (data.token != undefined) {
    ...
    var prmRep = {
      ...
      QFiltro:
        operMenu.operacion.registro === 'todos'
          ? this.QFiltro
          : ' PLANES.ID_PLAN = ' + this.FPlanes.ID_PLAN + ' ',
    };
  ...
  operGrid(e, operacion) {
    switch (operacion) {
      case 'new':
        ...
        break;
      case 'edit':
        const index = this.gridPlanesFin.instance.getRowIndexByKey(
          this.filasSelecc[0],
        );
  ...
  onRowValidating(e: any) {
    ...
    //   }
    // }
    // 1. Combinamos de forma segura el estado anterior con los nuevos cambios de la fila
    const dataEvaluated = { ...e.oldData, ...e.newData };

    // 2. Validación obligatoria para todas las filas: Tipo de Condición
    // if (!dataEvaluated.ID_CONDICION || dataEvaluated.ID_CONDICION === '') {
    //   showToast('Debe seleccionar un Tipo de Condición en la fila.', 'error');
    //   e.isValid = false;
    //   this.rowApplyChanges = true;
    //   this.esVisibleSelecc = 'none';
    ...
    // 2. Validaciones exclusivas si el Plan es de tipo 'Constante'
    if (this.FPlanes.TIPO_FRECUENCIA === 'Constante') {
      if (!dataEvaluated.ID_TASA || dataEvaluated.ID_TASA === '') {
        showToast('Debe seleccionar una Tasa Financiera.', 'error');
        e.isValid = false;
        this.setEstadosControl(true, false, true, 'none'); // Bloqueo seguro de controles
        return;
      }
      if (!dataEvaluated.PAGO || dataEvaluated.PAGO <= 0) {
        showToast('Debe seleccionar el número de Pago.', 'error');
        e.isValid = false;
        this.setEstadosControl(true, false, true, 'none'); // Bloqueo seguro de controles
        return;
      }
    ...
    // 3. Si llegó aquí, la fila pasó los filtros y es válida
    e.isValid = true;
    this.setEstadosControl(false, false, true, 'always'); // Libera los controles visuales
  }

  // Captura los cambios de datos de cualquier campo del formulario de forma segura
  onFormDefFieldDataChanged(e: any) {
    if (e.dataField === 'ID_PLAN') {
      // Ejecutamos tu lógica de negocio original mandándole el formato que espera
      this.onValueChanged({ value: e.value }, 'ID_PLAN');

      // Forzamos al validador a evaluar el campo en tiempo real para pintar la línea roja
      setTimeout(() => {
        this.formPlanesFin.instance.validate();
      }, 100);
    }
  }

  // Función auxiliar para no repetir código de botones en cada 'if'
  private setEstadosControl(
    applyChanges: boolean,
    edit: boolean,
    rowNew: boolean,
    visibleSelecc: string,
  ) {
    this.rowApplyChanges = applyChanges;
    this.rowEdit = edit;
    this.rowNew = rowNew;
    this.esVisibleSelecc = visibleSelecc;
  }
  ...
  onInitNewRowPro(e: any) {
    ...
    e.data.ID_CONDICION = '';
    e.data.ID_TASA = '';
    e.data.DIFERIR_CI = 'No';
    e.data.DESCONTAR_CI = 'No';
  }
    ...
    if (e.value !== null && e.value !== undefined && e.value !== '') {
      switch (campo) {
        case 'ID_TASA':
          ...
          this.gridPlanesFin.instance.cellValue(
            cellInfo.rowIndex,
            'ID_TASA',
            e.value,
          );
  ...
  onCondicSelecc(event: any, cellInfo: any): void {
    ...
    // Si el usuario deselecciona la condición, asignar un valor vacío o nulo según tu lógica de negocio
    cellInfo.setValue(event.selectedItem?.ID_CONDICION || event.value);
  }
}
