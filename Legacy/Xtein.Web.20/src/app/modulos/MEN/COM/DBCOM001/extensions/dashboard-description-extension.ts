...
var modelDashboard: any = {
    control: Dashboard.DashboardControl,
    modo: String,
...
function onDashboardTitleToolbarUpdated(args) {
    ...
    // Switch entre viewer y designer
    args.options.actionItems.push({  
        type: "button",  
        icon: "dashboard-designer",  
        hint: "Modo diseño",  
        click: function () {  
            if (modelDashboard.modo === 'viewer'){
                modelDashboard.modo = "designer";
                modelDashboard.control.switchToDesigner();
            }
            else {
                modelDashboard.modo = "viewer";
                modelDashboard.control.switchToViewer();
            }

            // modelDashboard.forcePanelVisible = !modelDashboard.forcePanelVisible;
            // var PnlExt = modelDashboard.control.findExtension('dashboard-panel') as Dashboard.DashboardPanelExtension;

            // if (modelDashboard.forcePanelVisible)
            //     PnlExt.hidePanelAsync({} as any).done((e: any) => {
            //         modelDashboard.control.surfaceLeft(e.surfaceLeft);
            //     });
            // else
            // PnlExt.showPanelAsync({} as any).done((e: any) => {
            //     modelDashboard.control.surfaceLeft(e.surfaceLeft);
            // });
        }  
    });  
...
export class DashboardDescriptionExtension {
    ...
    constructor(private dashboardControl: Dashboard.DashboardControl, private adminDash) {
        this.menuItem = this.createMenuItem()
        modelDashboard.control = dashboardControl; 
        modelDashboard.modo = "viewer";
    ...
    start() {
        ...
        // modelDashboard.forcePanelVisible = false;
        // setTimeout(() => {
        //     var PnlExt = modelDashboard.control.findExtension('dashboard-panel') as Dashboard.DashboardPanelExtension;
        //     PnlExt.hidePanelAsync({} as any).done((e: any) => {
        //         modelDashboard.control.surfaceLeft(e.surfaceLeft);
