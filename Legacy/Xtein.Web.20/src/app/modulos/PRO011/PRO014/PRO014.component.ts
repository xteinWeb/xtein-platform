...
export class PRO014Component implements OnInit {
  ...
  async opPrepararGuardar(accion: string) {
    ...
    console.log(this.prmUsrAplBarReg.aplicacion);
    ...
    console.log('resultado de integridad' + resInte);
    // if(!resInte) return;
    ...
    if (accion === "filtro") {
      ...
      this._rutasdpservice
        ...
        .subscribe((resp: any) => {
          ...
          console.log('resultado de integridad' + datares[0]  );
  ...
  private reordenarRutasSimples(): void {
    ...
    if (!this.rutasPartes) return;

    setTimeout(() => {
      try {
        const seleccionadosSet = new Set(this.lisSelRutasSimples);
        const rutasSeleccionadas: any[] = [];
        const rutasNoSeleccionadas: any[] = [];

        for (const ruta of this.rutasPartes) {
          if (seleccionadosSet.has(ruta.ID_RUTA)) {
            rutasSeleccionadas.push(ruta);
          } else {
            rutasNoSeleccionadas.push(ruta);
          }
        }
        ...
        console.error('Error reordenando rutas:', error);
      } finally {
        this.loadingVisible = false;
      }
    }, 0);
  ...
  onSelecRutasSimples(e: any, opcion = 'secundaria'): any {
    ...
    if(this.mnuAccion.match('new|update')) {
      ...
      if ((e.addedItems !== null) && (e.addedItems !== undefined) && (e.addedItems.length > 0)) {
        ...
        if (opcion !== 'principal') {
          keys.forEach((elr: any) => {
            if (!this.lisSelRutasSimples.includes(elr.ID_RUTA)) {
        ...
        const ultimaRuta = keys[keys.length - 1];
        const cntRutas = this.seccNodos.filter(n => 
          n.id.toString().match('cnt_' + ultimaRuta.ID_RUTA + '_')
        );
        if (cntRutas.length > 0) {
          this.showModal('Ya fue añadida esta ruta!');
          const ID_RUTA_A_QUITAR = keys[keys.length - 1].ID_RUTA;
          this.lisSelRutasSimples = this.lisSelRutasSimples.filter(id => id !== ID_RUTA_A_QUITAR);
          return
        }
      ...
      if ((e.removedItems !== null) && (e.removedItems !== undefined) && (e.removedItems.length > 0)) {
        const remkeys = e.removedItems;
        const IDs_A_REMOVER = remkeys.map((elr: any) => elr.ID_RUTA);
        // Reconstruye la lista de seleccionados (Inmutable)
        this.lisSelRutasSimples = this.lisSelRutasSimples.filter(id => !IDs_A_REMOVER.includes(id));
  ...
  agregarLayout(IdRuta: string, NomRuta: string, opcion: string, selRuta: any) {
    ...
    if (msg_rep !== '') {
      ...
      // this.treeList.instance.selectRows([IdRuta], false);
      this.lisSelRutasSimples = [...this.lisSelRutasSimples.filter(ruta => ruta !== IdRuta)];
      return;
    }
