  ...
  onSelectionChangedPlan(e:any, component:any) {
    if (component === 'GRID') {
      if (e.selectedRowKeys.length > 0) {
        ...
        this.FECHA_INICIO = this.formatFecha(new Date(e.selectedRowsData[0].FECHA_INICIO));
        this.FECHA_FIN = this.formatFecha(new Date(e.selectedRowsData[0].FECHA_FINAL));
        this.FECHA_DIA = null;
        ...
        items.push(
          {
            ...
            onClick: () => {
              ...
              this.popupData = {...e.row.data, FECHA_ANTERIOR: e.row.data.FECHA_PROGRAMADA};
          ...
          },
          {
            ...
            onClick: () => {
              ...
              this.popupData = {...e.row.data, FECHA_ANTERIOR: e.row.data.FECHA_PROGRAMADA};
              this.selectPlanPopup = [e.row.data.ID_PLAN];
              this.FECHA_INICIO_POPUP = this.FECHA_INICIO;
              this.FECHA_FIN_POPUP = this.FECHA_FIN;
              this.FECHA_DIA_POPUP = e.row.data.FECHA_PROGRAMADA;
              this.popupVisible = true;
            }
          }
        );
    ...
    if (accion === 'cancelar' || accion === 'reprogramar') {
      ...
      try {
        const newData:any = await this.opPrepararGuardar(
          accion === 'cancelar' ? 'CANCELAR': accion === 'reprogramar' ? 'REPROGRAMAR' : '',
          (this.popupAccionGrid === 'DGPMP' ? 'PEDIDOS' : this.popupAccionGrid === 'DGproductos' ? 'AGENDA' : ''),
          [{...datos, FECHA_PROGRAMADA: new Date(this.FECHA_DIA_POPUP) }]
        );

        if (this.popupAccionGrid === 'DGPMP') {
          if (newData[0].FECHA_PROGRAMADA >= this.FECHA_INICIO && newData[0].FECHA_PROGRAMADA <= this.FECHA_FIN) {
            var pos:any = this.DGPMP.findIndex((d: any) => {
              const fechaA = d.FECHA_PROGRAMADA ? new Date(d.FECHA_PROGRAMADA) : null;
            ...
            if (pos !== -1) {
              this.DGPMP[pos].CANTIDAD_PROGRAMADA = newData[0].CANTIDAD_PROGRAMADA;
              this.DGPMP[pos].CANTIDAD_PENDIENTE = newData[0].CANTIDAD_PENDIENTE;
              this.DGPMP[pos].TOTAL = newData[0].TOTAL;
              this.gridProductosPMP.instance.refresh();
            } else {
              this.DGPMP.push(newData[0]);
              this.gridProductosPMP.instance.clearSelection();
              this.gridProductosPMP.instance.refresh();
              this.TOTAL_PENDIENTE = this.DGPMP.reduce((acc: number, item: any) => acc + (item.TOTAL || 0), 0);
            }
            ...
            const pos2:any = this.DGPMP.findIndex((d: any) => 
