...
export class PRO028Component {
        ...
        case 'OPERATIVA':
          ...
          if (this.DSecciones.findIndex((d:any) => d.ITEM === secc.ANTERIOR) > -1 && secc.MANO_OBRA) {
            ...
            if (infoCap > -1) {
              ...
              secc.EFECTIVIDAD_AJUSTADA = (newArray.ITEMS[infoCap].EFECTIVIDAD_AJUSTADA !== undefined && newArray.ITEMS[infoCap].EFECTIVIDAD_AJUSTADA !== null) ? newArray.ITEMS[infoCap].EFECTIVIDAD_AJUSTADA : 1;
              secc.HORAS_AJUSTADA = newArray.ITEMS[infoCap].HORAS_AJUSTADA ? newArray.ITEMS[infoCap].HORAS_AJUSTADA : secc.HORAS_DISPONIBLE;
              this.DSecciones.push(secc);
            }
          }
          break;
  ...
  onFechaDesde(e:any) {
    // Solo validamos si hay un evento de usuario (evita dispararse al cargar registros)
    if(this.mnuAccion.match('new|update') && e.event) {
	...
	onFechaHasta(e:any) {
    // Solo validamos si hay un evento de usuario
    if(this.mnuAccion.match('new|update') && e.event) {
      if((e.value === null) || (e.value === '') || (e.value === undefined)){
        showToast('Seleccione fecha', 'error');
        return;
  ...
  private lastValidatedDates: string = '';
    ...
    // Crear un identificador único para el par de fechas actual
    const currentDatesKey = `${a}_${b}`;
    
    // Si las fechas son las mismas que la última validación exitosa, no llamamos a la API
    if (this.lastValidatedDates === currentDatesKey) {
      return true;
    }
    ...
    if (f1 > f2){
      ...
      if (msg[0].ErrMensaje !== ''){
        ...
        this.lastValidatedDates = ''; // Limpiar caché si falla
        return false;
      } else {
        this.lastValidatedDates = currentDatesKey; // Guardar en caché
        return true;
      }
    }
	}
  ...
  convertirTiempoADecimal(hora:any) {
    ...
    return Math.round(horas + minutos / 60);
  }
  ...
  async validarCargarDatos() {
    if ( (this.FCapacidad.FECHA_FIN > this.FCapacidad.FECHA_INICIO) && 
      ...
      const nuevasSecciones: any[] = [];
      const mapaItemsAgregados = new Map<number, any>();

      // Primero procesamos Plantas y Procesos
      for (let secc of this.DSecciones_prev) {
        if (secc.TIPO === 'Planta') {
          if (this.FCapacidad.PLANTAS.includes(secc.ID_SECCION)) {
            nuevasSecciones.push(secc);
            mapaItemsAgregados.set(secc.ITEM, secc);
          }
        } else if (secc.TIPO === 'PROCESO') {
          if (mapaItemsAgregados.has(secc.ANTERIOR)) {
            nuevasSecciones.push(secc);
            mapaItemsAgregados.set(secc.ITEM, secc);
          }
        }
      ...
      // Identificamos las secciones operativas para consulta masiva
      const seccionesOperativas = this.DSecciones_prev.filter(secc => 
        secc.TIPO === 'OPERATIVA' && 
        mapaItemsAgregados.has(secc.ANTERIOR) && 
        secc.MANO_OBRA
      );

      if (seccionesOperativas.length > 0) {
        try {
          // Aseguramos que HORARIOS sea un arreglo (a veces viene como string JSON)
          let horariosArr = this.FCapacidad.HORARIOS;
          if (typeof horariosArr === 'string') {
            try { horariosArr = JSON.parse(horariosArr); } catch (e) { horariosArr = []; }
          }
          if (!Array.isArray(horariosArr)) horariosArr = [];

          // Calculamos horas productivas totales del horario seleccionado
          let contHorasPro = 0;
          horariosArr.forEach((ele: any) => {
            const Hrio = this.DTurnos.find((d: any) => d.ID_HORARIO === ele);
            if (Hrio) contHorasPro += this.convertirTiempoADecimal(Hrio.HR_PRODUCTIVA);
          });

          // Preparamos payload masivo
          const prm = {
            ITEMS: seccionesOperativas.map(secc => ({
              SECCION_H: secc.ID_SECCION,
              SECCION_P: (this.DSecciones_prev.find(d => d.ITEM === secc.ANTERIOR) || {}).ID_SECCION
            })),
            HORARIOS: horariosArr,
            C_HORAS: contHorasPro
          };

          const apiRest = this.sData.valideCapacidad('VAL CAPACIDAD MASIVA', prm, 'PRO028');
          const res = await lastValueFrom(apiRest);
          const results = JSON.parse(res.data);

          // Procesamos resultados masivos
          seccionesOperativas.forEach((secc) => {
            const infoCap = results.find((r: any) => r.SECCION_H === secc.ID_SECCION);
            if (infoCap) {
              const seccExistente = this.DSecciones.find(d => d.ITEM === secc.ITEM);
              
              secc.HORAS_HORARIO = infoCap.C_HORAS ? this.formatearHora(infoCap.C_HORAS) : this.formatearHora(contHorasPro);
              secc.N_TRABAJADORES_INSTALADA = infoCap.N_CAPACIDAD || 0;
              secc.HORAS_INSTALADA = infoCap.HrProyectadas ? this.formatearHora(infoCap.HrProyectadas) : '00:00';
              
              secc.N_TRABAJADORES_DISPONIBLE = seccExistente ? seccExistente.N_TRABAJADORES_DISPONIBLE : (infoCap.N_EMPLEADOS || 0);
              const hHorarioDecimal = infoCap.C_HORAS || contHorasPro;
              secc.HORAS_DISPONIBLE = this.formatearHora(hHorarioDecimal * secc.N_TRABAJADORES_DISPONIBLE);
              
              secc.EFECTIVIDAD_AJUSTADA = seccExistente ? seccExistente.EFECTIVIDAD_AJUSTADA : 1;
              const horasDispDecimal = hHorarioDecimal * secc.N_TRABAJADORES_DISPONIBLE;
              secc.HORAS_AJUSTADA = this.formatearHora(horasDispDecimal * secc.EFECTIVIDAD_AJUSTADA);
          ...
          showToast('Error al cargar capacidades masivas', 'error');
        }
      }

      this.DSecciones = nuevasSecciones;
      this.activeListaSeccion = this.DSecciones.length > 0;
      this.validandoDatos = false;
      this.loadingVisible = false;
  ...
  async getCapacidadHorario(secc:any) {
    ...
    if (msg[0].ErrMensaje !== ''){
      ...
      return {
        ...
        EFECTIVIDAD_AJUSTADA: 1,
        HORAS_AJUSTADA: this.formatearHora(0)
      };
    } else {
      return {
        ...
        EFECTIVIDAD_AJUSTADA: 1,
        HORAS_AJUSTADA: this.formatearHora(msg[0].HrDisponible)
      };
    }
  }
  ...
  onValueChangedValor(e:any, cellInfo:any) {
    if( this.mnuAccion.match('new|update')) {
      ...
      switch (cellInfo.column.dataField) {
        case 'N_TRABAJADORES_DISPONIBLE':
          const nTrabajadores = e.value || 0;
          const horasHorarioDecimal = this.convertirTiempoADecimal(data.HORAS_HORARIO || '00:00');
          const nuevasHorasDisponibleDecimal = nTrabajadores * horasHorarioDecimal;
          const nuevasHorasDisponibleStr = this.formatearHora(nuevasHorasDisponibleDecimal);

          // Recalcular también las horas ajustadas con la efectividad actual
          const efectividadActual = data.EFECTIVIDAD_AJUSTADA || 0;
          const nuevasHorasAjustadaDecimal = nuevasHorasDisponibleDecimal * efectividadActual;
          const nuevasHorasAjustadaStr = this.formatearHora(nuevasHorasAjustadaDecimal);

          // Actualizar objeto de datos directamente para asegurar sincronización con DSecciones
          data.N_TRABAJADORES_DISPONIBLE = nTrabajadores;
          data.HORAS_DISPONIBLE = nuevasHorasDisponibleStr;
          data.HORAS_AJUSTADA = nuevasHorasAjustadaStr;

          // Actualizar UI de la grilla
          this.treeListSecciones.instance.cellValue(rowIndex, 'N_TRABAJADORES_DISPONIBLE', nTrabajadores);
          this.treeListSecciones.instance.cellValue(rowIndex, 'HORAS_DISPONIBLE', nuevasHorasDisponibleStr);
          this.treeListSecciones.instance.cellValue(rowIndex, 'HORAS_AJUSTADA', nuevasHorasAjustadaStr);
          this.conCambios++;
          break;

        case 'EFECTIVIDAD_AJUSTADA':
          let valorEfectividad = e.value || 0;
          // Si el usuario escribe un número como 90, lo convertimos a 0.9
          if (valorEfectividad > 1) {
            valorEfectividad = valorEfectividad / 100;
          }
          
          const horasDisponiblesStr = data.HORAS_DISPONIBLE || '00:00';
          const horasDecimal = this.convertirTiempoADecimal(horasDisponiblesStr);
          const resultadoDecimal = horasDecimal * valorEfectividad;
          const hFormat = this.formatearHora(resultadoDecimal);
          
          // Actualizar objeto de datos directamente
          data.EFECTIVIDAD_AJUSTADA = valorEfectividad;
          data.HORAS_AJUSTADA = hFormat;

          // Actualizar UI de la grilla
          this.treeListSecciones.instance.cellValue(rowIndex, 'EFECTIVIDAD_AJUSTADA', valorEfectividad);
          this.treeListSecciones.instance.cellValue(rowIndex, 'HORAS_AJUSTADA', hFormat);
          this.conCambios++;
          break;
      
        default:
          data[cellInfo.column.dataField] = e.value;
          this.treeListSecciones.instance.cellValue(rowIndex, cellInfo.column.dataField, e.value);
          this.conCambios++;
          break;
      }
    }    
