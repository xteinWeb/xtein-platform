  ...
  opIrARegistro(accion: string): void {
    ...
    switch (accion) {
      ...
      case "Eliminado":
        ...
        if (this.VDatosReg.length > 0) {
          let nx = this.prmUsrAplBarReg.r_numReg - 1;
          if (nx < 0) nx = 0;
          this.FPlanes = this.VDatosReg[nx];
        } else {
          this.opBlanquearForma();
    ...
    try {
      ...
      // ...existing code...
      if (this.FPlanes && this.FPlanes.FECHA_INICIO && !(this.FPlanes.FECHA_INICIO instanceof Date)) {
        this.FPlanes.FECHA_INICIO = new Date(this.FPlanes.FECHA_INICIO);
      }
      if (this.FPlanes && this.FPlanes.FECHA_FINAL && !(this.FPlanes.FECHA_FINAL instanceof Date)) {
      ...
      this.sData.save(this.mnuAccion, prmDatos, 'PRO-033').subscribe((resp) => {
        ...
        if (res[0].ErrMensaje !== "") {
          ...
          if (this.mnuAccion === 'new') {
            ...
            this.showModal(res[0].InfoCapacidad, 'Aviso');
