...
export class VEN230Component implements OnInit {
  ...
  calcularValores(){
    ...
    this.DPrefactura.TOTAL = this.DPrefactura.SUB_TOTAL + this.TOT_GRAV;
  ...
  valoresObjetos(obj: string, opcion: any = undefined) {
    ...
    if (obj === 'condicion' || obj === 'ref') {
      this._sdatos.consulta('CONDICIONES LISTAS',
        ...
        if(this._sdatos.accion === 'r_buscar' || this._sdatos.accion === 'r_numreg'){
          const cond = this.DCondicionLista.find(e => e.CODIGO === this.DPrefactura.ID_CONDICION);
          if (cond) {
            this.TIPO_CONDLISTA = cond.TIPO;
            if (cond.PLAZOS) {
              this.DPlazos = JSON.parse(cond.PLAZOS);
            }
            this.valoresObjetos('productos');
          }
        }
