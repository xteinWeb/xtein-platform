...
export class ADM015Service {
  ...
  public D_AplicacionesAsociadas: any;
  ...
  consulta(accion: any, prmDatos: any, aplicacion: any): Observable<any> {
    ...
    return this.http
      .post<any>(url, body, { headers: { 'Content-Type': 'application/json' } })
      .pipe(
        map((vec: any) => {
          return vec;
        }),
  ...
  // Consultas varias
  consultaAplicacionesAsociadas(accion: any, prmDatos: any, aplicacion: any): Observable<any> {
    const prmJ = {
      prmAccion: accion,
      prmDatos: JSON.stringify(prmDatos),
      prmConexion: { EMPRESA: localStorage.getItem('empresa') },
    ...
    const body = JSON.stringify(prmJ);
    aplicacion = aplicacion.replace('-', '');
    let url = this.endPoint + '/' + aplicacion + '/consultaAplAso';
    return this.http
      .post<any>(url, body, { headers: { 'Content-Type': 'application/json' } })
      .pipe(
        map((vec: any) => {
