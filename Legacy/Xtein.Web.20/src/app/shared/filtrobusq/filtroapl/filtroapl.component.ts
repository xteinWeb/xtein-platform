...
export class FiltroaplComponent implements OnInit {
  ...
  gridBoxValue: string[] = [];
  gridSeleccObj: any[] = [];
  dropDownOptions = { width: 600, height: 400 };
  camposFiltro: string[] = [];
  ...
  calculateNombreODescripcion = (rowData: any) => {
    return rowData ? (rowData.NOMBRE || rowData.DESCRIPCION || '') : '';
  }
      ...
      .subscribe({ 
        next: (data: any)=> {
          ...
          if (this.camposFiltro && this.camposFiltro.length > 0) {
            this.DFiltro = this.DFiltro.filter((filt: any) => this.camposFiltro.includes(filt.CAMPO));
          }
          ...
          this.DFiltro.forEach((filt:any) => {
            if (filt.DATA) {
              ...
              // Si es otra lista
              if (filt.DATA_TYPE != "searchbox" && filt.DATA_TYPE != "lista")
                filt.DATA = JSON.parse(filt.DATA);
  ...
  cargaDatosLista(campo: string, data: any) {
    const customStore = new CustomStore({
      ...
      load: (loadOptions: any) => {
        ...
        // Extraer búsqueda del filtro si searchValue está vacío (ocurre al buscar en el panel de dx-data-grid)
        if (!searchValue && loadOptions.filter) {
          const extractSearch = (filterExpr: any): string => {
            if (!filterExpr) return '';
            if (typeof filterExpr === 'string') return filterExpr;
            if (Array.isArray(filterExpr)) {
              if (filterExpr.length === 3 && typeof filterExpr[2] === 'string' && (filterExpr[1] === 'contains' || filterExpr[1] === 'startswith' || filterExpr[1] === '=')) {
                return filterExpr[2];
              }
              for (const item of filterExpr) {
                if (Array.isArray(item)) {
                  const val = extractSearch(item);
                  if (val) return val;
                }
              }
            }
          ...
          searchValue = extractSearch(loadOptions.filter);
        }
        ...
        return lastValueFrom(
          this._sgenerales.consulta('listaQuery', prm, 'generales')
        ).then((result) => {
          const parsedData = JSON.parse(result.data);
          let totalCount = 0;
          if (parsedData && parsedData.length > 0) {
            totalCount = parsedData[0].TotalCount || parsedData[0].totalCount || parsedData.length;
          }
          return {
            data: parsedData,
      ...
      byKey: (key: any) => {
        ...
        return lastValueFrom(
          this._sgenerales.consulta('listaQuery', 1, key)
        ).then(result => result.data[0]);
      }
    });

    this.dataSources["D_"+campo] = customStore;
    this.dataSources["Grid_D_"+campo] = new DataSource({
      store: customStore,
      paginate: true,
      pageSize: 50
    });
  ...
  ngOnInit(): void {
    this.eventsSubscription = this.events.subscribe((datos: any) => {
      if (datos.accion == 'activar') {
        ...
        this.camposFiltro = datos.CamposFiltro || [];
