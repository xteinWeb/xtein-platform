...
export class PRO03403Component implements OnInit, OnDestroy {
    ...
    toggleExpand() {
        ...
        setTimeout(() => {
            if (this.gridPedidosPMP) this.gridPedidosPMP.instance.updateDimensions();
        }, 100);
    }
    ...
    loadData(planData?: any): void {
        ...
        this.sData.consulta('PEDIDOS PMP', prm, 'PRO-023').subscribe((data: any) => {
            ...
            if (res[0].ErrMensaje && res[0].ErrMensaje !== '') {
                ...
                // Total is Programmed + Pending (Excluding CANCELADO)
                const activeItems = res.filter((d: any) => d.ESTADO !== 'CANCELADO');
                this.totalCantidad = activeItems.reduce((acc: number, d: any) =>
                    acc + (parseFloat(d.CANTIDAD_PROGRAMADA || d.CANTIDAD) || 0), 0
                );

                // Filter pending orders (following PRO023 logic: ESTADO === 'PEDIDO')
                this.DGPMP = res.filter((d: any) => d.ESTADO === 'PEDIDO');
                this.cantidadPendiente = this.DGPMP.reduce((acc: number, d: any) => acc + (parseFloat(d.CANTIDAD) || 0), 0);

                // Calculate programmed quantity matching PRO03402 filter (anything with ORDEN_PRO or not in PEDIDO/CANCELADO state)
                const programmedItems = res.filter((d: any) =>
                    (d.ORDEN_PRO !== null && d.ORDEN_PRO !== undefined && d.ORDEN_PRO !== '') ||
                    (d.ESTADO !== 'PEDIDO' && d.ESTADO !== 'CANCELADO')
                );
                // Ensure unique ITEM ID for selection (as in PRO023)
                this.DGPMP.forEach((d: any, index: number) => {
                    ...
                    // Respect backend values for quantities if they exist
        ...
        try {
            ...
            for (let i = 0; i < res.length; i++) {
                ...
                if ((res[i].ESTADO === 'PEDIDO' || res[i].ESTADO === 'REGISTRADO' || res[i].ESTADO === 'PENDIENTE') &&
                    ...
                    try {
                        const prmSave = { PRODUCTOS: [res[i]], USUARIO: this.USUARIO_LOCAL, TIPO: 'Manual' };
                        const saveRes: any = await lastValueFrom(this.sData.save('GUARDAR AGENDA', prmSave));
                        const parsedSave = JSON.parse(saveRes.data);
                        if (parsedSave[0].ErrMensaje === '') {
            ...
            // 3. Full reload to ensure consistency
            this.loadData();
    ...
    onCellPrepared(e: any): void {
        // Solo deja seleccionables las que tengan Matriz Completa
    ...
    async onConfirmarAccionPopup(accion: string, datos: any) {
        if (accion === 'cancelar' || accion === 'reprogramar') {
            ...
            try {
                ...
                const prm = { PRODUCTOS: [datosCompletos], USUARIO: this.USUARIO_LOCAL, TIPO: 'PEDIDOS' };
                this.loadingVisible = true;
