...
import {
  DxButtonModule, DxDataGridModule, DxDropDownBoxModule, DxFormComponent, DxFormModule, DxLoadPanelModule,
  DxSelectBoxModule,
  DxTabsModule,
  DxTagBoxModule,
  DxTextBoxModule,
...
import { clsClientes } from './clsVEN209.class';
...
import { VEN20901Component } from './VEN20901/VEN20901.component';
...
@Component({
  selector: 'app-VEN209',
  templateUrl: './VEN209.component.html',
  styleUrls: ['./VEN209.component.css'],
  standalone: true,
  imports: [MatTabsModule, DxLoadPanelModule, DxFormModule, DxSelectBoxModule,
    ...
    DxTagBoxModule, DirtelinfoComponent, VEN20901Component, DxValidatorModule,
    DxTabsModule, FiltrobusqComponent, GeninformesComponent, VistarapidaComponent]
})
export class VEN209Component {
  ...
  tabsInfoAcreedores: string[] = ['Identificación', 'Ubicaciones', 'Financieros', 'Datos Ventas', 'Histórico'];

  FAcreedores: clsClientes;
  ...
  ngOnInit(): void {
    ...
    this.FAcreedores = new clsClientes();
  ...
  onSeleccIdLegal(e: any) {
    ...
    if (e !== 'refrescar') {
      ...
      if (dnomprov) {
        ...
        this.FAcreedores.TIPO_ID = dnomprov.TIPO_ID;
  ...
  onSeleccClase(e: any): void {
    ...
    this.FAcreedores.CLASE = (e.value || '').toUpperCase();
    ...
    switch (operMenu.accion) {
      ...
      case 'r_cancelar':
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            const esNuevoRegistro = this.mnuAccion === 'new';
            const esModificacion = this.mnuAccion === 'update';
            ...
            // Si era un nuevo registro y no hay registros previos, blanquear
            if (
              esNuevoRegistro &&
              ...
              // Notificar a componentes hijos que se blanqueó
              this.eventsSubjectDirTelInfo.next({ operacion: 'nuevo' });
              this.eventsSubjectCondProv.next({ operacion: 'nuevo' });
            } else if (esModificacion && this.FAcreedores_prev) {
              // Si era una modificación, restaurar los valores originales
              this.FAcreedores = JSON.parse(JSON.stringify(this.FAcreedores_prev));
              // Restaurar también los valores visuales
              this.selectRepresentante = [this.FAcreedores.REPRESENTANTE_LEGAL];
              this.selectIdLegal = [this.FAcreedores.ID_LEGAL];
              this.selectGrupo = [this.FAcreedores.ID_GRUPO];
              // Recargar datos en componentes hijos
              this.valoresObjetos('todos');
            } else {
              // Si había registros, volver al registro actual
              this.opIrARegistro('r_numreg');
            }
          }
        });
      ...
      case 'r_imprimir':
        var prmRep = {
          ...
          QFiltro:
            operMenu.operacion.registro === 'todos'
              ? this.QFiltro
              : " CLIENTES.ID_CLIENTE = '" + this.FAcreedores.ID_CLIENTE + "'",
        };
        ...
        if (operMenu.operacion.modo === 'email') {
          let template =
            this.especAplicacion.find((e) => e.NOMBRE_OBJETO === 'TEMPLATE')
              ?.VALOR_DEFECTO ?? '';
          let asunto =
            this.especAplicacion.find((e) => e.NOMBRE_OBJETO === 'ASUNTO EMAIL')
              ?.VALOR_DEFECTO ?? '';
          let email_sale =
            this.especAplicacion.find(
              (e) => e.NOMBRE_OBJETO === 'EMAIL USUARIO'
            )?.VALOR_DEFECTO ?? '';
          let nom_usr =
            this.especAplicacion.find(
              (e) => e.NOMBRE_OBJETO === 'EMAIL USUARIO'
  ...
  async opPrepararModificar() {
    ...
    // Guardar una copia de los valores originales antes de modificar
    this.FAcreedores_prev = JSON.parse(JSON.stringify(this.FAcreedores));

    // Valida la existencia
    this.activarEdicion('activo');
    const prm = { ID_CLIENTE: this.FAcreedores.ID_CLIENTE, accion: 'integridad referencial' };
  ...
  ValidaDatos(Accion: string): boolean {
    var msg = '';
    if (Accion === "requerido") {
      if (this.FAcreedores.ID_CLIENTE === '') msg += 'Id Acreedor ,';
      if (this.FAcreedores.ID_LEGAL == null) msg += 'Id Legal,';
      if (this.FAcreedores.NOMBRE === null || this.FAcreedores.NOMBRE === '') msg += 'Primer Nombre,';
      if (this.FAcreedores.APELLIDO === null || this.FAcreedores.APELLIDO === '') msg += 'Primer Apellido,';
      if (this.FAcreedores.APELLIDO2 === null || this.FAcreedores.APELLIDO2 === '') msg += 'Segundo Apellido,';
      if (this.FAcreedores.TIPO_ID === null || this.FAcreedores.TIPO_ID === '') msg += 'Tipo de Identificación,';
      if (this.FAcreedores.PERSONA === null || this.FAcreedores.PERSONA === '') msg += 'Persona Nat/Jur,';
      if (this.FAcreedores.ID_GRUPO === '') msg += 'Grupo Cliente ,';
      if (this.FAcreedores.PERFIL_TRIBUTARIO === '') msg += 'Perfil Tributario ,';
      if (this.FAcreedores.RT.length == 0) msg += 'Representación tributaria ,';
      if (this.FAcreedores.CLASE === '') msg += 'Clase Cliente ,';
  ...
  async opPrepararGuardar(accion: string): Promise<void> {
    ...
    // Validar existencia del ID_CLIENTE antes de guardar (para new y update)
    if (this.mnuAccion === 'new' || this.mnuAccion === 'update') {
      const resultadoValidacion = await this.ValideExistencia({
        value: this.FAcreedores.ID_CLIENTE
      });
      ...
      .subscribe((resp) => {
        ...
        if (res[0].ErrMensaje !== "") {
          ...
          if (this.mnuAccion === 'new') {
            this.QFiltro = " CLIENTES.ID_CLIENTE = '" + this.FAcreedores.ID_CLIENTE + "'"
            this.prmUsrAplBarReg = {
              ...this.prmUsrAplBarReg,
              error: "",
              accion: "r_navegar",
  ...
  AccionEliminar(): void {
    ...
    const prm = {  ID_CLIENTE: this.FAcreedores.ID_CLIENTE, USUARIO: this.prmUsrAplBarReg.usuario };
    this._sdatos
  ...
  opIrARegistro(accion: string): void {
    ...
    try {
      ...
      // Asegurar que PERSONA tenga un valor válido
      if (!this.FAcreedores.PERSONA) this.FAcreedores.PERSONA = '';
      ...
      this.DClientesPro = this.FAcreedores["CLIENTES_PRO"];
      ...
      this.eventsSubjectCondProv.next({
        operacion: 'consulta',
        dataSource: {
          ADICIONALES: this.DCondiciones.ADICIONALES ?? {},
          CONDICIONES: this.DCondiciones.CONDICIONES ?? [],
          CLIENTES_PRO: this.DClientesPro ?? [],
          BANCOS: this.DBancos ?? []
        }
      });
  ...
  opVista(): void {
    // Valida que existan datos para mostrar
    if (!this.VDatosReg || this.VDatosReg.length === 0) {
      showToast('No hay datos para mostrar. Primero realice una búsqueda.', 'warning');
      return;
    }
  ...
  async ValideExistencia(e: any) {
    ...
    if (this.mnuAccion === 'update' && this.FAcreedores_prev.ID_CLIENTE === e.value) {
      return (true);
    }
    ...
    if (res[0].ErrMensaje === '') {
      ...
      // Mostrar Swal.fire en lugar de toast
      Swal.fire({
        title: 'Cliente ya existe',
        text: `El cliente con identificación ${e.value} ya está registrado en el sistema. No se puede usar esta identificación.`,
        iconHtml: "<i class='icon-cancelar-ol error-color'></i>",
        confirmButtonColor: '#0F4C81',
        confirmButtonText: 'Entendido',
        allowOutsideClick: false
      });
      var fNextEditor: any = this.formAcree.instance.getEditor('ID_CLIENTE');
      fNextEditor.focus();
  ...
  onSeleccTipoIdLegal(e) {
    this.FAcreedores.TIPO_ID = e.value;
  }
  onSeleccPersona(e) {
  ...
  onTabChanged(e: any) {
    switch (e.itemIndex) {
      case 0:
        this.tabVisible = ["block", "none", "none", "none", "none"];
        break;
      case 1:
        this.tabVisible = ["none", "block", "none", "none", "none"];
        break;
      case 2:
        this.tabVisible = ["none", "none", "block", "none", "none"];
        break;
      case 3:
        this.tabVisible = ["none", "none", "none", "block", "none"];
        break;
      case 4:
        this.tabVisible = ["none", "none", "none", "none", "block"];
        break;
      default:
        break;
    }
  ...
  valoresObjetos(obj: string, opcion: any = undefined) {
    ...
    if (obj == 'id_legal' || obj == 'todos') {
      ...
      this._sdatos.consulta('ID LEGALES', prm, "ADM-205").subscribe((data: any) => {
        ...
        this.eventsSubjectSboxIdLegal.next({
          dataSource: this.DIdLegal,
          valueExpr: "ID_LEGAL",
    ...
    if (obj == 'zona' || obj == 'todos') {
      let prm = { TIPO_UBICACION: "ZONA" }
      this._sdatos.consultaZonas('consulta',prm).subscribe((data: any) => {
          const res = JSON.parse(data.data);
          if ((data.token != undefined)) {
            const refreshToken = data.token;
            localStorage.setItem("token", refreshToken);
    ...
    if (obj == 'adc' || obj == 'todos') {
      let prm = { ESTADO: "ACTIVO" }
      this._sdatos.consultaADC('ADC',prm).subscribe((data: any) => {
          const res = JSON.parse(data.data);
          if ((data.token != undefined)) {
            const refreshToken = data.token;
            localStorage.setItem("token", refreshToken);
    ...
    if (obj == 'status' || obj == 'todos') {
      let prm = { STATUS: {} }
      this._sdatos.consultaSTATUS('STATUS',prm).subscribe((data: any) => {
          const res = JSON.parse(data.data);
          if ((data.token != undefined)) {
            const refreshToken = data.token;
            localStorage.setItem("token", refreshToken);
    ...
    if (obj === 'grupos' || obj === 'todos') {
      const prm = { ID_GRUPO_PADRE: 'DEUDORES' };
