...
export class VEN212Component implements OnInit {
  ...
  onSelectionCondicion(e) {
    ...
    if (e.selectedRowsData != null && e.selectedRowsData.length > 0){
      ...
      if(this.DFactura.TIPO_VENTA == 'CREDITO'){
  ...
  onSeleccTipoVenta(e){
    ...
    this.DPlazos = this.DFactura.TIPO_VENTA != 'CREDITO' ? [{PLAZO: 0}] : this.DPlazos;
    if (this.DFactura.TIPO_VENTA == 'CREDITO')
  ...
  async onRespuestaSettings(e: any) {
    ...
    if (e.titulo == 'Cargar Pedido' || e.titulo == 'Cargar Solicitud') {
      // Total seleccionado
