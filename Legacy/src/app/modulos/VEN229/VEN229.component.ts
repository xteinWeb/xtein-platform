...
export class VEN229Component implements OnInit {
  ...
  async onChangeCantidad(rowData: any, value: any, currentRowData: any) {
    ...
    if (datRes[0].ErrMensaje !== "") {
      ...
      rowData.PORC_IVA = porIva; // Usar directamente el porcentaje sin multiplicar por 0.19
  ...
  async onSeleccRowProducto(e, cellInfo, datacompo) {
    ...
    if (e.selectedRowKeys[0]) {
      if (!this.especRepetirItem &&
        this.DPedidoItems.findIndex((d: any) => d.PRODUCTO === e.selectedRowKeys[0]) != -1) {
      ...
      if (dnompro) {
        ...
        const porcentajeIVA = tasaIvaGen ? tasaIvaGen.PORCENTAJE : 0;
        cellInfo.data.PORC_IVA = porcentajeIVA;
  ...
  onValueChangedUMVenta(e, cellInfo) {
    ...
    if (this.DPedidoItems.findIndex((d: any) => d.PRODUCTO === cellInfo.data.PRODUCTO && d.UDM_VENTA === e.value) === -1) {
      if (dum) {
        ...
        cellInfo.data.SUB_TOTAL = cellInfo.data.CANTIDAD * cellInfo.data.VALOR_UNITARIO;
        const porIva = cellInfo.data.GRAVAMENES.PORCENTAJE !== undefined ? cellInfo.data.GRAVAMENES.PORCENTAJE : 0;
        cellInfo.data.VALOR_IVA = (cellInfo.data.SUB_TOTAL * cellInfo.data.PORC_IVA) / 100;
  ...
  calcularValores() {
    this.DPedido.SUB_TOTAL = this.DPedidoItems.reduce((n, { SUB_TOTAL }) => n + SUB_TOTAL, 0);
  ...
  setValoresCartera() {
    this.DPedido.VALOR_CUOTA = (this.DPedido.TOTAL - this.DPedido.CUOTA_INICIAL) / this.DPedido.PLAZO
  }
  ...
  onSeleccIvas(e) {
    ...
    this.gridPedidoItems.instance.cellValue(rowIndex, "PORC_IVA", e.addedItems[0].PORCENTAJE / 100);
    this.gridPedidoItems.instance.cellValue(rowIndex, "TOTAL", canPro * valUnit + valIva);
    ...
    if (nx !== -1) {
      ...
      this.DPedidoItems[nx].TOTAL = canPro * valUnit + valIva;
  ...
  valoresObjetos(obj: string, opcion: any = undefined) {
    ...
    if (obj === 'gravamenes' || obj === 'ref') {
      ...
      if (opcion !== 'original')
        prm = {
          PEDIDO: this.DPedido,
          ITEMS: this.DPedidoItems, APL_GRAV: opcion
        };
      else
        prm = {
          PEDIDO: this.DPedido,
          ITEMS: this.DPedidoItems, OPCION_GRAV: opcion
        };
      this._sdatos
        ...
        .subscribe((data: any) => {
          ...
          if (opcion !== undefined) {
            this.DPedidoItems.forEach(item => {
              ...
              if (nx > -1) {
                item.VALOR_IVA = this.DPedidoGravAux[nx].VALOR_IVA;
                item.SUB_TOTAL = this.DPedidoGravAux[nx].SUB_TOTAL;
                item.TOTAL = this.DPedidoGravAux[nx].TOTAL;
                item.GRAVAMENES = this.DPedidoGravAux[nx].GRAV_TOTAL;
                item.GRAV_TOTAL = this.DPedidoGravAux[nx].GRAV_TOTAL;
