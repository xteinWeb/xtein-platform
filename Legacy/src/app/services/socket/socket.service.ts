    ...
    this.wsocket.onmessage = (event) => {
      ...
      switch (response.topic) {
        case 'response_recodatorio':
          ...
          break
        case 'notificaciones':
          let usuariosRec = response.data.data.USUARIO_REC.map((u: string) => u.trim());
          let usuarioEnv = response.data.data.USUARIO_ENV.ID_RESPONSABLE.trim();
          let usuarioLogueado = this.USUARIO_LOCAL.trim();
