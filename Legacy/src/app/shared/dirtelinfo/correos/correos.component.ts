...
export class CorreosComponent {
  ...
  initNewRow(e){
    // Ya no se permite agregar filas manualmente
    // Los tipos de correo vienen predefinidos desde la API
    e.cancel = true;
    showToast('Los tipos de correo están predefinidos. Solo complete el email correspondiente.', 'info');
  }
  ...
  insertingRow(e) {
    // Ya no se permite insertar filas nuevas
    e.cancel = true;
  }
  ...
  operGrid(e, operacion) {
    switch (operacion) {
      case 'new':
        // Ya no se permite agregar nuevas filas
        showToast('Los tipos de correo están predefinidos. Solo complete el email correspondiente.', 'info');
        break;
      case 'edit':
        this.esEdicion = true;
      ...
      case 'cancel':
        ...
        break;
      case 'delete':
        // Ya no se permite eliminar filas predefinidas
        showToast('No se pueden eliminar los tipos de correo predefinidos.', 'warning');
        break;
      ...
      .subscribe((data: any) => {
        ...
        if (res && Array.isArray(res) && res.length > 0) {
          res.forEach((item: any, index: number) => {
            ...
            // Buscar si existe un dato para este tipo de correo
            const datoExistente = datosExistentes.find(d => d.ETIQUETA === tipoCorreo);
            
            const nuevoEmail = new clsEmail();
            nuevoEmail.ITEM = item.ITEM || (index + 1);
            nuevoEmail.ETIQUETA = tipoCorreo; // Tipo de email viene en VALOR1
            
            if (datoExistente) {
              // Si existe dato, usar los valores existentes
              nuevoEmail.EMAIL = datoExistente.EMAIL || '';
            } else {
