...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, DxListModule, DxDateBoxModule, DxButtonModule, DxLoadPanelModule,
            DxToolbarModule, GES_INFOComponent]
})
...
opMenuRegistro(operMenu: clsBarraRegistro): void {
  ...
  orderGroupFechas() {
    this.notificationsLeidas = [
      { key: 'Hoy', items: [] },
      { key: 'Anteriores', items: [] }
    ];
    this.notificationsNoLeidas = [
      { key: 'Hoy', items: [] },
      { key: 'Anteriores', items: [] }
    ];
    // Grupos de fechas
    ...
    const leidasHoy:any = grupoHoy.filter((d:any) => d.ESTADO === "Visto");
    const lediasAnteriores:any = grupoAnteriores.filter((d:any) => d.ESTADO === "Visto");
    this.notificationsLeidas = [
      { key: 'Hoy', items: leidasHoy.length > 70 ? leidasHoy.slice(0, 70) : leidasHoy },
      { key: 'Anteriores', items: lediasAnteriores.length > 70 ? lediasAnteriores.slice(0, 70) : lediasAnteriores }
    ];

    const noLeidasHoy:any = grupoHoy.filter((d:any) => d.ESTADO === "Enviado");
    const noLeidasAnteriores:any = grupoAnteriores.filter((d:any) => d.ESTADO === "Enviado");
    this.notificationsNoLeidas = [
      { key: 'Hoy', items: noLeidasHoy.length > 70 ? noLeidasHoy.slice(0, 70) : noLeidasHoy },
      { key: 'Anteriores', items: noLeidasAnteriores.length > 70 ? noLeidasAnteriores.slice(0, 70) : noLeidasAnteriores }
    ];

    this.loadingVisible = false;
    setTimeout(() => {
      if (this.mostrarNtf === 'Leidas' || this.mostrarNtf === 'No leidas')
