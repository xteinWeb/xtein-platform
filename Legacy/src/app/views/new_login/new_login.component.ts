...
export class NewLoginComponent {
  ...
  async onChangePassword(e: any, prm: any) {
    ...
    switch (prm) {
      ...
      case 'aceptar':
        if (this.digitos.length < 5) {
          showToast('Digite todos los números del código.', 'error');
          break;
        }
        if (this.digitos.length > 6) {
          showToast(
            'El código ingresado excede los digitos deben ser solo Seis(6)).',
            'error'
          );
          break;
        } else {
