...
import { findAdmApplication } from '../applications/adm-application.registry';

/** Native Federation entry point consumed by the existing Shell workspace host. */
@Component({
  selector: 'xtein-adm-application-host',
  standalone: true,
  imports: [],
...
export class ApplicationHostComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  private async loadRequestedApplication(): Promise<void> {
    ...
    const registration = findAdmApplication(applicationId);
    if (!registration) {
    ...
    try {
      ...
      console.error('XTEIN ADM application resolution failed.', { applicationId, error });
    } finally {
