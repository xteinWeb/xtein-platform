...
import { findInvApplication } from '../applications/inv-application.registry';

/** Native Federation entry point consumed by the existing Shell workspace host. */
@Component({
  selector: 'xtein-inv-application-host',
  standalone: true,
  imports: [],
...
export class ApplicationHostComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  private async loadRequestedApplication(): Promise<void> {
    ...
    const registration = findInvApplication(applicationId);
    if (!registration) {
    ...
    try {
      ...
      console.error('XTEIN INV application resolution failed.', { applicationId, error });
    } finally {
