...
import { findMalmApplication } from '../applications/malm-application.registry';

/** Native Federation entry point consumed by the existing Shell workspace host. */
@Component({
  selector: 'xtein-malm-application-host',
  standalone: true,
  imports: [],
...
export class ApplicationHostComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  private async loadRequestedApplication(): Promise<void> {
    ...
    const registration = findMalmApplication(applicationId);
    if (!registration) {
    ...
    try {
      ...
      console.error('XTEIN MALM application resolution failed.', { applicationId, error });
    } finally {
