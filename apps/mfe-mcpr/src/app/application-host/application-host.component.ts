...
import { findMcprApplication } from '../applications/mcpr-application.registry';

/** Native Federation entry point consumed by the existing Shell workspace host. */
@Component({
  selector: 'xtein-mcpr-application-host',
  standalone: true,
  imports: [],
...
export class ApplicationHostComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  private async loadRequestedApplication(): Promise<void> {
    ...
    const registration = findMcprApplication(applicationId);
    if (!registration) {
    ...
    try {
      ...
      console.error('XTEIN MCPR application resolution failed.', { applicationId, error });
    } finally {
