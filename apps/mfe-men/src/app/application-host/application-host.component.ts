...
import { findMenApplication } from '../applications/men-application.registry';

/** Native Federation entry point consumed by the existing Shell workspace host. */
@Component({
  selector: 'xtein-men-application-host',
  standalone: true,
  imports: [],
...
export class ApplicationHostComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  private async loadRequestedApplication(): Promise<void> {
    ...
    const registration = findMenApplication(applicationId);
    if (!registration) {
    ...
    try {
      ...
      console.error('XTEIN MEN application resolution failed.', { applicationId, error });
    } finally {
