...
import { findMfinApplication } from '../applications/mfin-application.registry';

/** Native Federation entry point consumed by the existing Shell workspace host. */
@Component({
  selector: 'xtein-mfin-application-host',
  standalone: true,
  imports: [],
...
export class ApplicationHostComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  private async loadRequestedApplication(): Promise<void> {
    ...
    const registration = findMfinApplication(applicationId);
    if (!registration) {
    ...
    try {
      ...
      console.error('XTEIN MFIN application resolution failed.', { applicationId, error });
    } finally {
