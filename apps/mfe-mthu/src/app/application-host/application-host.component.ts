...
import { findMthuApplication } from '../applications/mthu-application.registry';

/** Native Federation entry point consumed by the existing Shell workspace host. */
@Component({
  selector: 'xtein-mthu-application-host',
  standalone: true,
  imports: [],
...
export class ApplicationHostComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  private async loadRequestedApplication(): Promise<void> {
    ...
    const registration = findMthuApplication(applicationId);
    if (!registration) {
    ...
    try {
      ...
      console.error('XTEIN MTHU application resolution failed.', { applicationId, error });
    } finally {
